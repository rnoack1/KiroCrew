...
rather than restarting. A row that cannot be PARSED at all is skipped and warned about
like any other, but is carried verbatim into the next `loops` payload rather than being
dropped: it arms nothing, and any cause that rewrites the store — a legacy token
migration among them — would otherwise delete a neighbour nobody chose to remove.
An unreadable sidecar refuses every write in that process
