...
`folder_id_for_restore(..., prune_unknown=False)` for a SHAPE check alone; that was removed,
...
### Scope: restore-time validation runs on every startup path
Read this before the sweep protocol below, because the two have different blast radii. The
sweep is confined to the two delete handlers. The **validation** is not: every path that
rebuilds a slot from persisted metadata routes its `folder_id` through
`DashboardState.folder_id_for_restore` and its `tags` through `tag_ids_for_restore` —
history restore, resume, fork, and channel arrival alike. So a **malformed** id is dropped on
**every startup**, not only when a delete is in flight.

Which rejection acts depends on the caller, and the split is the point — see "Two dispositions"
below for the full rationale. The MALFORMED (non-`str`) rejection is unconditional everywhere.
The **four cold-start paths validate nothing at all**: they adopt the persisted value verbatim,
so a readable-but-stale `folders.json` cannot unfile a filing made after that snapshot. Do not
read this section as licence to prune at boot — restoring that would reintroduce the silent
durable mass-unfile, which is the harm this whole split exists to avoid.
...
`tag_ids_for_restore` takes no `prune_unknown`, so a readable-but-stale `tags.json` parses as
...
That the remaining eight callers prune is deliberate, and it is a behaviour change rather than a
...
- **`_FORCE_SAVE_CLOBBER_SITES = 10`** — an UPPER BOUND, not an equality. A new `force=True`
  slot-metadata save fails it; removing one does not, because fewer sites is strictly safer
  and reddening the PR that deletes a clobber site would penalise the outcome this ratchet
  exists to encourage. A companion test asserts the census is non-empty, so the bound cannot
  pass vacuously if the detector stops matching. That is what keeps the deferred layer
  decision from quietly growing, and why the count lives in a test rather than only in prose.
...
rather than chosen here.** `tag_ids_for_restore` takes no `prune_unknown` parameter, so a
