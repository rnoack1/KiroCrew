...
**A run whose START cannot be recorded does not happen.** Every script-hook run writes a `started`
invocation row before the command and an outcome row on every completion path, and the two fail in
...
because by the time one is written the command has already run and refusing proves nothing. This is
the same split the MCP dispatch path uses, and for the same stated reason — SEL's criterion is
ATTENDEDNESS, and a hook fires with nobody watching.
...
turn-lifecycle events fire inside an attended turn: their `started` row is still written critically,
so a failure is visible, but it does NOT deny the run. Extending refusal to them would have traded a
...
| folder inheritance on slot CREATE (`chat_handlers`, the `validate_folder_tag_ids` apply) | **no — known gap, deferred not justified** | the tags land on a slot that did not exist a moment earlier, so there is no lane to move FROM; but a subscriber watching for arrival in a lane does want a session created directly in it, and `added` / `removed` can express that (`added=[id]`, `removed=[]`). Emitting here is correct and is NOT done — it needs two more files, so it waits for the choke point |
...
| **any writer above, called with an APP token** | **no — known gap** | `_lane_dispatch_is_permitted` admits dashboard callers only, so an app-driven lane move writes the tag and fires nothing — see below |
