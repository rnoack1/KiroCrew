...
So the rule is directional, not symmetric: a marker may be released early but never held past its successor, while a note may be held but never released early. `markers_only=True` is therefore correct exactly at a seam that owes a note to a turn that has not arrived yet but is about to write rows the marker must precede — the two current callers are the stage-execution exit and the synthesis path, each passing the predicate that detects a structural successor (`chat_runner.py`). A new seam calling `flush_deferred_notes` with no argument holds BOTH, which silently moves any pending boundary; a new seam passing `markers_only=True` unconditionally delivers notes early. Both are defects, and neither fails visibly, which is why the mode belongs here rather than only in the call sites. This is the one sanctioned qualification of the RFC's "no second notion of held": the hold is still one list with one flush, but its membership is no longer one kind of thing.
