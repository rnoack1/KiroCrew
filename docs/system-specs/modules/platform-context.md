...
- `CapabilityManager` (operations-based external package/capability manager).
  ...
  **No opt-out ships with this change, deliberately.** A config switch was written and
  ...
  It is SEVERABLE, and deliberately so. Both package reads go through ONE helper in
  `handlers/prompts.py`, `_pinned_package_read`, which is where the gated call lives; the
  key grammar lives entirely in `handlers/_shared.py`, and this change adds no grammar
  symbol to `prompts.py` at all. So the remedy for a dedup-install breakage is to make that
  one helper call `read_skill_file`, which returns the base read behaviour and leaves every
