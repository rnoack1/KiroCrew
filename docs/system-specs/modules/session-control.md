...
Redundancy therefore holds only for the server-versus-server half of the question, and that
half is not separable from the rest without losing the per-read binding. `dashboardSlice.ts`
records the same conclusion at `CloseTombstone`, naming the two cases that sit outside the
stamp's authority. Both mechanisms are kept deliberately, and the cost of carrying both is
accepted here rather than deferred again.
