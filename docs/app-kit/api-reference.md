...
> **BEHAVIOUR CHANGE — `ephemeral` now defaults to `false`, so an omitted flag PERSISTS.** It previously defaulted to `true`, which meant a caller that left it out got memory-only content that a tab close discarded. Durability is the point of the queue, so the default inverted for EVERY caller, not only the artifact companion and Papyrus: content posted without the flag is now written to the session metadata line on disk. A caller that relies on content never reaching disk must now pass `ephemeral: true` explicitly — only the literal boolean `true` is honoured.
...
**A 200 means different things for the two halves, and the difference is the durability boundary.**

The **queued context** half is durable. It is persisted into the session's metadata line and re-seated when the session is restored, so it survives closing the tab and survives a gateway restart. `maxAge` continues to run while the session is closed (wall-clock), so a long-closed session does not reopen holding stale background context, and an entry with no expiry comes back indefinitely. This is a deliberate change to the previous contract, in which the queue was memory-only.

The **held visible line** is still memory-only. When a turn is already running the visible half waits in memory (`visibleDeferred: true`), and a gateway restart in that window drops it. So a 200 with `visibleDeferred: true` still promises ordering against the running turn rather than persistence.

**Compatibility — a caller that re-posts on reconnect will now double-inject.** The previous recipe was "if you need a note to survive a restart, re-post it". Under the durable queue, the restored copy and the re-post both drain into the next user message, so the model sees the content twice. Callers written to that recipe should drop the re-post for `/context` and for the context half of `/note`. Re-posting remains the only way to guarantee a *visible* line that was held when the gateway went down, and a caller that needs exactly-once visible delivery should key on its own idempotency marker rather than on the endpoint.
