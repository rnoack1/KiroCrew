...
window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (url.includes('/api/chat/tags')) {
    // `?tags=` selects the state to photograph: the empty board and the failed fetch
    // are reachable no other way, and each renders copy the default state never shows.
    const want = new URLSearchParams(window.location.search).get('tags')
    if (want === 'error') {
      return new Response('upstream unavailable', { status: 503 })
    }
    if (want === 'empty') {
      // Succeeds carrying no STATUS tag, which is the empty-board case.
      return new Response(JSON.stringify([{ id: 'cc33dd44ee55', name: 'urgent', status: false }]), {
        status: 200,
        headers: { 'content-type': 'application/json' },
      })
    }
