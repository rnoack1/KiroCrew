...
// Picked is the accent state, so it is the one where a neutral badge could go unreadable.
const pickedFirst = params.get('picked') === '1'
 ...
 */
const RAW = [
  '(recommended) Rebase onto the latest main and re-run the failing suite',
  'Run every check again from the beginning',
  'Skip the slow tests',
]

/**
 * The guard-declined arm. `/clear` is a harness command, so `applyMarkerGuards` REFUSES to
 * strip the marker and returns the label untouched -- which is the safe behaviour and also
 * the one state where a chip draws raw protocol text. Capturing it is the point: the frame
 * is the evidence for what the fallback rendering actually looks like, which is a thing a
 * reviewer has to see rather than be told.
 */
const RAW_DECLINED = [
  '(recommended) /clear',
  'Summarise the thread instead',
]

/**
 * A BARE command-shaped label, no marker anywhere. This is a behaviour change to chips that
 * already existed: `/clear` used to quick-send on one click and now fills the composer, with a
 * dashed edge and its own note as the only visible account of why it differs from a sibling.
 */
const RAW_BARE = [
  '/clear',
  'Summarise the thread instead',
...
/** Both kinds at once, which is the only state that renders the combined note. */
const RAW_MIXED = [
  '(recommended) /clear',
  '@deploy',
  'Summarise the thread instead',
...
const declinedMode = params.get('declined') === '1'
const bareMode = params.get('bare') === '1'
const mixedMode = params.get('mixed') === '1'
const parsedDeclined = parseOptions(`[OPTIONS: ${RAW_DECLINED.join(' | ')}]`)
const parsedBare = parseOptions(`[OPTIONS: ${RAW_BARE.join(' | ')}]`)
const parsedMixed = parseOptions(`[OPTIONS: ${RAW_MIXED.join(' | ')}]`)
const options = mixedMode
  ? parsedMixed.options
  : bareMode
    ? parsedBare.options
    : declinedMode
      ? parsedDeclined.options
      : fixOn
        ? parsed.options
        : RAW
const recommended = mixedMode
  ? parsedMixed.recommended
  : bareMode
    ? parsedBare.recommended
    : declinedMode
      ? parsedDeclined.recommended
      : fixOn
        ? parsed.recommended
        : undefined
      ...
      <div className="input-area px-4 pb-1 pt-1 mx-auto w-full flex flex-col" style={{ maxWidth: 'var(--mc-input-width, 900px)' }}>
        <div data-bar>
          <FollowUpBar
            ...
            picked={pickedFirst && options.length ? new Set([options[0]]) : new Set()}
            onSelect={() => {}}
            onSend={() => {}}
          />
        </div>
...
interface BadgeMeasure {
  ...
  /** The chip's own tooltip, so a claim about the hint is read rather than assumed. */
  chipTitle: string | null
  /** Whether the declined-state note is actually painted, not merely present. */
  keptNoteVisible: boolean
}
...
window.__measure = () => {
  ...
  // Case-INSENSITIVE: the badge renders `RECOMMENDED`, so a case-sensitive filter here
  // matched nothing and reported a missing badge that was on screen the whole time.
  const badges = Array.from(bar.querySelectorAll<HTMLElement>('span[data-testid="recommended-badge"]'))
    .filter(n => /recommended/i.test(n.textContent || ''))

  return {
    fix: fixOn ? 'on' : 'off',
    contentWidth,
    // The tooltip is where the kept-marker hint lands, so the harness reads it rather
    // than trusting that a title prop was wired.
    chipTitle: (label.closest('[title]') as HTMLElement | null)?.getAttribute('title') ?? null,
    // The declined state's explanation is a visible row note now, so the harness reads
    // whether a reader can SEE it rather than whether a tooltip carries it.
    keptNoteVisible: (() => {
      const n = document.querySelector<HTMLElement>('[data-testid="recommended-kept-note"]')
      if (!n) return false
