...
const CLOSE = { action: 'close', label: 'Nothing else, close this tab' }
const OPTIONS = ['Show me the diff', 'Run the tests first']

/**
 * Three <FollowUpBar> mounts, each in its own labelled panel so one image carries the
...
async function start() {
  ...
  createRoot(document.getElementById('root')).render(
    <div style={{ padding: 28, background: 'var(--bg)' }}>
      ...
      <Panel label="3 — blocked: composer holds unsent work, reason is visible">
        <FollowUpBar options={OPTIONS} picked={new Set()} onSelect={noop} onSend={noop}
