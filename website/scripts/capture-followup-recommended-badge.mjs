...
for (const theme of ['dark', 'light']) {
  for (const arm of ['off', 'on', 'picked']) {
    const fix = arm === 'off' ? 'off' : 'on'
    const picked = arm === 'picked' ? '1' : '0'
    const page = await browser.newPage({ viewport: VIEWPORT, colorScheme: theme })
    await page.goto(`${BASE}/capture/followup-recommended-badge.html?width=${WIDTH}&theme=${theme}&fix=${fix}&picked=${picked}`)
    ...
    // Named for the STATE each frame documents, not for a fix sequence.
    const name = `${arm === 'off' ? 'marker-inline-unbadged' : arm === 'picked' ? 'badge-on-picked-chip' : 'badge-beside-label'}-${theme}.png`
...
// The guard-declined state, per theme. A separate loop because its assertions are the
// INVERSE of the badged arm: the marker must still be in the label and no badge may
// exist, which is the safe outcome -- and the tooltip is what tells the user why.
for (const theme of ['dark', 'light']) {
  const page = await browser.newPage({ viewport: VIEWPORT, colorScheme: theme })
  await page.goto(`${BASE}/capture/followup-recommended-badge.html?width=${WIDTH}&theme=${theme}&declined=1`)
  await page.waitForSelector('[data-bar] .truncate')
  await page.waitForTimeout(ENTRANCE_SETTLE_MS)

  const m = await page.evaluate(() => window.__measure())
  const tag = `${theme}/declined`

  check(`${tag} the marker stays in the dispatched label`, m.markerInLabel === true, `markerInLabel=${m.markerInLabel}`)
  check(`${tag} no badge is rendered for a declined marker`, m.badgeCount === 0, `badgeCount=${m.badgeCount}`)
  check(`${tag} the kept prefix is explained VISIBLY, not on hover`, m.keptNoteVisible === true, `keptNoteVisible=${m.keptNoteVisible}`)

  await page.screenshot({ path: `${OUT}/marker-kept-on-guarded-label-${theme}.png` })
  console.log(`     shot ${OUT}/marker-kept-on-guarded-label-${theme}.png`)
  await page.close()
}

// The two states no frame covered: a BARE command chip (a behaviour change to chips that
// already shipped) and a menu carrying both kinds, which is the only state whose note is
// combined. Asserted on the note's own text so a frame cannot pass showing the wrong note.
for (const [mode, file, needle] of [
  ['bare', 'bare-command-chip', 'instead of being sent'],
  ['mixed', 'mixed-hold-note', 'instead of being sent'],
]) {
  for (const theme of ['dark', 'light']) {
    const page = await browser.newPage({ viewport: VIEWPORT, colorScheme: theme })
    await page.goto(`${BASE}/capture/followup-recommended-badge.html?width=${WIDTH}&theme=${theme}&${mode}=1`)
    await page.waitForSelector('[data-bar] .truncate')
    await page.waitForTimeout(ENTRANCE_SETTLE_MS)

    const noteText = await page.evaluate(() => {
      const el = document.querySelector('p[data-testid$="-note"]')
      return el ? el.textContent || '' : ''
    })
    const noteCount = await page.evaluate(
      () => document.querySelectorAll('p[data-testid$="-note"]').length,
    )
    const tag = `${theme}/${mode}`

    check(`${tag} a note is on screen, not only a tooltip`, noteText.length > 0, `len=${noteText.length}`)
    check(`${tag} exactly one note renders`, noteCount === 1, `noteCount=${noteCount}`)
    check(`${tag} the note states the guarantee`, noteText.includes(needle), noteText.slice(0, 90))
    check(
      `${tag} the note does not enumerate triggers`,
      !noteText.includes('“/”') && !noteText.includes('“@”'),
      noteText.slice(0, 90),
    )

    await page.screenshot({ path: `${OUT}/${file}-${theme}.png` })
    console.log(`     shot ${OUT}/${file}-${theme}.png`)
    await page.close()
  }
}
