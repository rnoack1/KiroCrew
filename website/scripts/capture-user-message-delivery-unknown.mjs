...
const run = async () => {
  ...
  for (const theme of ['dark', 'light']) {
    ...
    // The strip carried no screenshot at all, so no reviewer had seen an exit label or the
    // consequence note. Self-checked on the same terms as the bubbles above.
    const strips = await page.$$eval('[data-capture-strip] [role="status"]', (n) => n.length)
    const labels = await page.$$eval(
      '[data-capture-strip] [data-testid^="delivery-exit-"]',
      (nodes) => nodes.map((n) => n.textContent?.trim() || ''),
    )
    const notes = await page.$$eval('[data-capture-strip] p[id^="capture-note-"]', (nodes) =>
      nodes.map((n) => n.textContent?.trim() || ''),
    )
    const distinct = new Set(labels).size
    if (strips !== 4 || labels.length !== 4 || distinct !== 4 || notes.length !== 1) {
      console.error(`  FAIL ${theme}: strip expected 4 captions/4 distinct labels/1 note, saw `
        + `${strips}/${labels.length} (${distinct} distinct)/${notes.length} ${JSON.stringify(labels)}`)
      failed += 1
      await ctx.close()
      continue
    }
    const stripFile = `${OUT}/delivery-strip-${theme}.png`
    await page.locator('[data-capture-strip]').screenshot({ path: stripFile })
    console.log(`  ok   ${theme} -> ${stripFile} (labels: ${JSON.stringify(labels)})`)
    console.log(`       note: ${JSON.stringify(notes[0])}`)
    await ctx.close()
  }
  await browser.close()
  if (failed) {
