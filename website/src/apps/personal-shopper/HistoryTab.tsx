...
function ProductRow({
  ...
  return (
    <div className="flex items-center gap-2 text-xs px-3 py-2 rounded-lg bg-[var(--bg-elevated)]">
      ...
      {price != null && (
        <span className="text-[var(--accent)] font-semibold">${price}</span>
      )}
...
function FeedbackBadge({ feedback }: { feedback: string }) {
  const config: Record<string, { label: string; icon: typeof ThumbsUp; className: string }> = {
    purchased: {
      label: 'purchased',
      icon: ShoppingCart,
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full font-medium ${c.className}`}>
      <Icon size={10} />
      {c.label}
    </span>
  )
