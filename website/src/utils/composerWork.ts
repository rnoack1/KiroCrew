...
/**
 * Does this work include a category NO store can answer for?
 *
 * `knowledge`, `uploading` and `voiceCapture` are the three the doc comments above
 * already single out as leaving no trace in any collection — and none has a persisted
 * representation either, so no draft store can answer for them once the window holding
 * them stops reporting.
 *
 * That is what decides how long a cross-window dirty claim may live. Where the work is
 * recoverable, a short expiry is safe because the persisted copy still answers
 * afterwards; here the claim IS the only record, and a browser stops a frozen tab's
 * timers, so expiring it on the refresh scale discards work that is still on screen in
 * a window that simply could not say so.
 */
export function hasUnrecoverableComposerWork(
  work: Pick<ComposerWork, 'knowledge' | 'uploading' | 'voiceCapture' | 'files' | 'sessionRefs'>,
): boolean {
  // `files` and `sessionRefs` persist to sessionStorage, which is PER TAB: another window
  // cannot read them, so from its side they are as invisible as work stored nowhere.
  return work.knowledge || work.uploading || work.voiceCapture
    || work.files.length > 0 || work.sessionRefs.length > 0
}

/**
 * Can ANOTHER window recover this work if the composer's window goes quiet?
 *
 * Two independent conditions, and both must hold. The work KIND must have a shared-storage
 * representation — the predicate above rules out the per-tab and unstored kinds — and THIS
 * host must actually write one, which the work object cannot express: an embedded pane keeps
 * its composer in component state and persists nothing at all.
 *
 * `persistedCrossWindow` defaults to the protective answer, so a host that says nothing is
 * treated as storing nothing. Getting this wrong in the other direction is what hands a
 * frozen pane's draft the refresh-scale claim TTL and lets another window delete it.
 */
export function workIsCrossWindowRecoverable(
  work: Pick<ComposerWork, 'knowledge' | 'uploading' | 'voiceCapture' | 'files' | 'sessionRefs'>,
  persistedCrossWindow: boolean | undefined,
): boolean {
  return persistedCrossWindow === true && !hasUnrecoverableComposerWork(work)
}
