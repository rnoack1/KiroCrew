...
const reclaimUnderQuota = (keep: string): number => {
  let freed = 0
  try {
    // A LIVE session's derived cache re-derives only by discarding what the reader can see,
    // so it is spent last, after every candidate belonging to no live session.
    const foreign: string[] = []
    const liveDerived: string[] = []
    const isLive = (k: string): boolean => {
      const id = sessionIdOf(k)
      return Boolean(id && liveInstances.has(id))
    }
    for (let i = 0; i < localStorage.length; i += 1) {
      const k = localStorage.key(i)
      if (!k || k === keep) continue
      if (isDerivedCacheKey(k)) {
        (isLive(k) ? liveDerived : foreign).push(k)
        continue
      }
      ...
      if (isDerivedCacheKey(stamped) || stampedValue === null) foreign.push(k)
    }
    const drop = (keys: readonly string[]): number => {
      let n = 0
      for (const k of keys) {
        try {
    ...
    freed = drop(foreign)
    if (freed === 0) freed = drop(liveDerived)
...
type LiveIdentity = { stamp: string; incarnation?: string; generation?: number; epoch?: string }
...
export function noteLiveSessionInstances(
  slots: readonly { key: string; created?: string; incarnation?: string }[],
  order?: WireOrder,
): void {
  rememberOrder(order)
...
const writeWouldBeStale = (key: string): boolean => {
  ...
  // Equal stamps do NOT prove the same writer: a resume on the same key preserves `created`,
  // so the frame comparison below is what separates the incarnations and must be reached.
...
/** An opaque writer token: the durable stamp PLUS the instance that captured it. A resume on
 *  the same key preserves `created`, so a stamp alone cannot tell a live writer from a
 *  superseded one holding a token captured before the resume. */
export function sessionOwnerStamp(sessionId: string): string | undefined {
  const live = liveInstances.get(sessionId)
  if (!live) return undefined
  return `${live.stamp}|${live.incarnation ?? ''}`
}

/** True when `token` was captured by an instance the live one has superseded. */
/** True when `token` was captured by an instance the live one has superseded.
 *
 *  Keyed on the slot's `incarnation`, which is minted per live slot object. The wire
 *  GENERATION cannot serve here: it counts server EMISSIONS, so any holder that outlives one
 *  snapshot would read as superseded by its own session. */
const writerTokenIsStale = (token: string, live: LiveIdentity): boolean => {
  const [stamp, incarnation] = token.split('|')
  if (stamp !== live.stamp) return true
  // A token carrying no instance identity can only be checked on the stamp it was written
  // against, which is the pre-resume behaviour and no worse than it.
  if (!incarnation || live.incarnation === undefined) return false
  return incarnation !== live.incarnation
}
...
export function setSessionScopedItem(
  ...
  if (writerOwner !== undefined) {
    ...
    if (live && writerTokenIsStale(writerOwner, live)) {
