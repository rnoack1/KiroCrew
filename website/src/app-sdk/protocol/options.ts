...
import { readRecommendedIndex } from './recommendation'
...
export function parseOptions(content: string): ParsedOptions {
  ...
  if (!last || last.index === undefined)
    return { text: content, options: [], recommended: null, multi: true, isPlan: false }
  ...
  // Labels pass through UNTOUCHED: the recommendation is carried out of band, so what a
  // click dispatches is what the producer wrote. `filter(Boolean)` drops an empty label,
  ...
  const stripped = stripOptionMarkers(content)
  const text = stripped.trim()
  // Read the tag from the STRIPPED text, not from `content`: the producer rule puts the
  // tag on the line BEFORE the marker, so while the marker is still present the tag is
  // not at the tail and a tail-anchored read finds nothing. Same ordering the backend
  // uses (`extract_options` first, then `extract_recommended_index`).
  //
  // Out of range yields no recommendation rather than a badge on the wrong option: the
  // index and the menu are written by the same producer, so a mismatch is its bug and the
  // safe reading of a bug is "no recommendation".
  const recommendedIndex = readRecommendedIndex(stripped)
  const recommended =
    recommendedIndex !== null && recommendedIndex >= 1 && recommendedIndex <= options.length
