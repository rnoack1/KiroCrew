...
import { OPTION_MARKER_PATTERN_SOURCE, OPTION_ACTION_MARKER_PATTERN_SOURCE, MARKER_PATTERN_FLAGS, stripPartialOptionMarker, matchActionMarkers, stripOptionMarkers } from '../app-sdk/protocol/optionMarker'
...
describe('OPTION_ACTION_MARKER_RE grammar', () => {
  ...
  it('does not catastrophically backtrack, and carries no nested quantifier', () => {
    ...
    // An ordinary body character excludes every closer, so the body stops at the first
    // plausible one instead of running to the last on the line. A closer reaches the body
    // only through the two conditional arms below.
...
describe('entry parsing', () => {
  it('treats a `|` as label text, since the body is ONE entry', () => {
    expect(parseOptions('[OPTION-ACTIONS: close=First   |   close=Second]').action)
      .toEqual({ action: 'close', label: 'First   |   close=Second' })
  })

  it('yields NOTHING when the body opens with an unknown action name', () => {
    // `bogus=Nope|close=Close it` is one entry naming `bogus`, so no chip renders.
    expect(parseOptions('[OPTION-ACTIONS: bogus=Nope|close=Close it]').action).toBeNull()
  })

  it('still returns null when NO entry is recognized', () => {
    expect(parseOptions('[OPTION-ACTIONS: bogus=Nope|alsobad=Also]').action).toBeNull()
  })
  ...
  it('renders no chip for a pipe body, matching the shared corpus case', () => {
    expect(parseOptions('[OPTION-ACTIONS: reboot=Nope | close=Yes | exec=Nope]').action).toBeNull()
  })
  ...
  it('DROPS an entry with an empty label', () => {
    ...
    expect(parseOptions('[OPTION-ACTIONS: close=   ]').action).toBeNull()
    expect(parseOptions('[OPTION-ACTIONS: close=Keep | close=]').action)
      .toEqual({ action: 'close', label: 'Keep | close=' })
  })
  ...
  it('DROPS an entry with no `=` at all', () => {
    ...
    expect(parseOptions('[OPTION-ACTIONS: close]').action).toBeNull()
    expect(parseOptions('[OPTION-ACTIONS: Just a label | close=Real]').action).toBeNull()
  })
