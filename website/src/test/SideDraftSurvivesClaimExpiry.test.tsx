...
import { beginSlotQuiesce, slotHasUnsentWork } from '../utils/slotComposerRegistry'
...
describe('a side composer draft outlives its expiring claim', () => {
  ...
  it('is still discoverable after the claim ages past its TTL', () => {
    ...
    publishSlotDirty('composer-side-1', SLOT, true, true)
  ...
  it('persists IMMEDIATELY while the slot is quiescing, with no timer advanced', () => {
    // The close is committing. On the debounced path the archive lands first and the
    // deletion is irreversible, so a post-commit check can only report the loss.
    const release = beginSlotQuiesce(SLOT)
    renderHook(() => useSlotDraftPersistence(SLOT, 'typed inside the close round-trip'))
    // Deliberately NO advanceTimersByTime: the point is that the write already happened.
    // The store maps slot -> composer ids, so an entry existing IS the durable write.
    expect(loadSideDrafts()[SLOT] ?? []).toHaveLength(1)
    release()
  })

  it('still DEBOUNCES when no close is in flight, so the fast path is the exception', () => {
    renderHook(() => useSlotDraftPersistence(SLOT, 'ordinary typing, no close pending'))
