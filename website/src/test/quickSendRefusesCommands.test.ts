 ...
 *
 * Nothing to do with the recommendation, which is carried out of band and never touches a
 * label. This predicate stands on its own: a bare `/clear` chip carries no marker, so
 * quick-send fired it and the transcript was erased. The reachable chain is opt-in quick
 * send, an option reaching the menu from content the agent read, and one ordinary user click.
 */
describe('quick send refuses a label that would run rather than say', () => {
  ...
  it('SENDS a bare provenance opener, which is a deliberate narrowing', () => {
    // A provenance opener mislabels a message's ORIGIN; it does not run anything. It was
    // in this refusal only while a strip could EXPOSE one the raw label hid -- and there is
    // no strip now, so such a label is the producer's own visible text and the user reads it
    // before clicking. This is also what quick-send did before any of this existed.
    // Keeping it would mean carrying 28 literals plus a parity suite for a class that can no
    // longer be promoted.
    expect(sent('[Monitor wake]')).toBe('[Monitor wake]')
  })
  ...
  it('refuses an embedded skill token, the one form a leading test cannot see', () => {
    // The expander resolves `$name` ANYWHERE in the dispatched string.
    expect(sent('Run the $deploy skill')).toBeNull()
  })
