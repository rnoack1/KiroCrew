...
describe('action chip is blocked while the composer holds unsent work', () => {
  ...
  describe('the wiring, through ChatInput', () => {
    ...
    it('an ACTIVE VOICE CAPTURE disables the chip with an empty composer', () => {
      ...
      //
      // Delivered through the composer's voice SLICE, not a prop: chat-core P3-b
      // removed ChatInput's voice props, so a prop here would be ignored and this
      // would assert nothing.
      renderWithProviders(
        <ComposerVoiceSliceOverride inputProps={{ voiceCaptureActive: true }}>
          <ChatInput {...base} value="" />
    ...
    it('a TRANSCRIPTION still resolving also disables the chip', () => {
      ...
      renderWithProviders(
        <ComposerVoiceSliceOverride inputProps={{ voiceTranscribing: true }}>
          <ChatInput {...base} value="" />
        </ComposerVoiceSliceOverride>,
      )
      expect(chip()).toHaveAttribute('aria-disabled', 'true')
    ...
    it('negative control: no capture and no transcription leaves the chip live', () => {
      ...
      // else in this fixture disabling the chip. Goes through the same slice, so
      // that it fails if the override stops delivering — a control fed by an
      // ignored prop would pass while proving nothing.
      renderWithProviders(
        <ComposerVoiceSliceOverride
          inputProps={{ voiceCaptureActive: false, voiceTranscribing: false }}
        >
          <ChatInput {...base} value="" />
        </ComposerVoiceSliceOverride>,
      )
...
describe('hasUnsentComposerWork counts a derived boolean flag', () => {
  ...
  it('an active voice CAPTURE is unsent work, with nothing staged yet', () => {
    ...
    describe('the blocked reason must name a place the user can act on', () => {
      const reasonFor = (at: 'other-window' | 'unverifiable' | 'elsewhere') => {
        const view = render(
          <FollowUpBar
            options={[]} picked={new Set()} onSelect={vi.fn()}
            action={CLOSE} onAction={vi.fn()} actionBlockedBySlot={at}
          />,
        )
        const title = view.container.querySelector('[aria-disabled="true"]')?.getAttribute('title')
        view.unmount()
        return title ?? ''
      }

      it('gives another WINDOW its own sentence, not the pane one', () => {
        const window = reasonFor('other-window')
        expect(window).not.toBe(reasonFor('elsewhere'))
        // The pane wording sent the user to a side panel or split view that is not where
        // the draft is, so a close in another window could not be acted on at all.
        expect(window.toLowerCase()).toContain('window')
      })

      it('gives an UNVERIFIABLE claim its own sentence too', () => {
        const unverifiable = reasonFor('unverifiable')
        expect(unverifiable).not.toBe(reasonFor('elsewhere'))
        expect(unverifiable).not.toBe(reasonFor('other-window'))
      })

      it('keeps every short form inside the chip clip budget', () => {
        for (const at of ['other-window', 'unverifiable', 'elsewhere'] as const) {
          const view = render(
            <FollowUpBar
              options={[]} picked={new Set()} onSelect={vi.fn()}
              action={CLOSE} onAction={vi.fn()} actionBlockedBySlot={at}
          ...
          const short = view.container.querySelector('[data-testid="action-blocked-reason"]')
          if (short?.textContent) expect(short.textContent.length).toBeLessThan(45)
          view.unmount()
        }
      })
    })
