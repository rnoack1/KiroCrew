...
describe('unsent work summons its own close confirm, on every route', () => {
  ...
  it('ABORTS the close when another window holds a composer and the intent write FAILS', async () => {
    // Written straight to storage so it reads as ANOTHER window's presence: the module
    // tracks its own keys separately, so a key it never minted is somebody else's.
    localStorage.setItem('mc-slot-closing:present:another-window-1', String(Date.now()))
    ...
    const h = host()
    await act(async () => {
      await h.result.current.close(SLOT, { onError: (m: string) => errors.push(m) })
    })
    setItem.mockRestore()
    // The one window that could veto could not be ASKED, and every other read here is
    // local, so proceeding would delete a draft nothing was able to speak for.
    expect(mockDeleteSlot).not.toHaveBeenCalled()
    expect(errors).toContain(i18nT('hooks.useSessionActions.close_vetoed_unsent'))
  })

  it('still closes when the intent write fails and NO other window holds a composer', async () => {
    // Indexed, not `Object.keys`: jsdom's localStorage is not enumerable that way, so a
    // key-based loop silently does nothing and foreign presence survives into this case.
    for (let i = localStorage.length - 1; i >= 0; i -= 1) {
      const key = localStorage.key(i)
      if (key && key.startsWith('mc-slot-closing:present:')) localStorage.removeItem(key)
    }
    const setItem = vi.spyOn(Storage.prototype, 'setItem').mockImplementation(() => {
      throw new Error('storage disabled')
    })
