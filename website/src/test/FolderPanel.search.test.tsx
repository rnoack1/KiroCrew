...
describe('FolderPanel search', () => {
  ...
  it('names Refresh in the notice when the search can actually be retried', async () => {
    ...
    expect(await screen.findByRole('alert'))
      .toHaveTextContent('Search timed out — Refresh retries it')
  })
  ...
  it('gives the header control a VISIBLE Refresh label while the copy names it', async () => {
    ...
    // Not the accessible name, which was already there — the rendered text is what was missing.
    expect(refresh).toHaveTextContent('Refresh')
  })

  it('leaves the header control icon-only when no notice names it', async () => {
    renderPanel()
    await screen.findByText('README.md')
  ...
  it.each([
    ...
    expect(notice).not.toHaveTextContent('Refresh retries it')
  })
})
