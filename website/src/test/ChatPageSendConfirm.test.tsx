...
import { loadStagedSend, PANE_RECOVERY_KEY } from '../utils/chatPaneRecovery'
...
import { preSendStash, queuedSendStash, adoptPreSendStash } from '../hooks/useQueuedMessageActions'
...
let armedKnowledge: { items: { id: string; title: string; content: string; source?: string; tokens: number }[]; totalTokens: number } | null = null
vi.mock('../pages/chat/useKnowledgeFetch', async (importOriginal) => {
  ...
  return {
    ...actual,
    useKnowledgeFetch: () => ({
      results: [], query: '', loading: false, pendingKnowledge: armedKnowledge,
      searchKnowledge: vi.fn(), inject: vi.fn(), clearPending: vi.fn(), clearResults: vi.fn(),
    }),
  }
})
...
describe('send() confirms its own optimistic bubble from the response', { timeout: 20_000 }, () => {
  ...
  /* The test above is the LOSSLESS twin of this one, and released retention even before the fix,
   * so it measures the gate rather than the harness. */
  it('releases retention on a queued send whose payload was too rich to stash', async () => {
    armedKnowledge = {
      items: [{ id: 'k1', title: 'Rate limit design', content: 'Token bucket, 40 rps.', source: 'wiki', tokens: 12 }],
      totalTokens: 12,
    }
    sendChat.mockResolvedValue({ ok: true, json: () => Promise.resolve({ ok: true, queued: true, queue_id: 'q-rich' }) })
    const store = makeStore()
    await renderPage(store)
    await sendText('summarise this')

    await waitFor(() => expect(userRow(store)).toBeTruthy())
    await waitFor(() => expect(sendChat).toHaveBeenCalledTimes(1))
    expect(String(sendChat.mock.calls.at(-1)?.[0] ?? ''),
      'the knowledge block diverges the wire text, which is what makes this send unstashable').toContain('[KNOWLEDGE CONTEXT')
    await waitFor(() => expect(userRow(store)?.meta?.pendingServerRow,
      'a cancelled entry never drains into a page, so this release is the only one it gets').toBe(false))
  })
  ...
  it('KEEPS the restored composer when the ECHO confirms the send', async () => {
    ...
    // Reconciliation is now marked by `confirmedSendId` plus the loss of `optimistic`; the wire
    // `sendId` deliberately SURVIVES, because upstream's confirmed-send lookup reads it on the row.
  ...
  it('addresses a transport failure to the SENDING slot, not the active one', async () => {
    // Base already addresses both failure branches to the sending slot; this PINS that
    // against regression, since the unslotted `appendMessage` would write to the active one.
    sendChat.mockRejectedValue(new TypeError('Failed to fetch'))
    const store = makeStore()
...
describe('discard cannot reach context staged AFTER the restore', () => {
  it('withholds the affordance for a file staged DURING the pending send', async () => {
    ...
    await waitFor(() => expect(screen.queryByTestId('delivery-exit-discard'),
  ...
  it('withholds the affordance once a new file is attached', async () => {
    ...
    expect(await screen.findByTestId('delivery-exit-discard')).not.toBeNull()
...
describe('discard still retires the attachment the restore itself staged', () => {
  it('offers the affordance for a recovered send that carried a file', async () => {
    ...
    await waitFor(() => expect(screen.queryAllByText(/resending will send it twice/i).length).toBeGreaterThan(0))
    ...
    expect(screen.queryByText(/^Clear the composer$/i), 'discard belongs to the payload it restored')
      .not.toBeNull()
  })
})
...
describe('the duplicate-resend warning is echoed AT the composer', () => {
  it('shows it while a staged payload sits there, and drops it only once the payload is gone', async () => {
    ...
    expect(screen.queryAllByText(/send it twice/i).length).toBeGreaterThan(0)
    ...
    await waitFor(() => expect(screen.queryAllByText(/send it twice/i)).toHaveLength(0))
  })
  ...
  it('agrees with the bubble once confirmed, and offers a working discard', async () => {
    ...
    await waitFor(() => expect(userRow(store)?.meta?.deliveryUnknown).toBe(true))
    const sendId = userRow(store)?.meta?.sendId as string
    // A LATER send's confirm retires this one's doubt marker; then it confirms too.
    ...
    const discard = screen.getByText(/^Clear the composer$/i)
    await act(async () => { fireEvent.click(discard) })
    await waitFor(() => expect((screen.getByLabelText('Message input') as HTMLTextAreaElement).value).toBe(''))
    expect(screen.queryAllByText(/send it twice/i)).toHaveLength(0)
  })
...
describe('a restored payload stays MARKED across a reload', () => {
  it('re-arms the caption from the persisted marker, so a reload cannot resend it unmarked', async () => {
    ...
    await waitFor(() => expect(screen.queryAllByText(/send it twice/i).length).toBeGreaterThan(0))
  })
})
...
describe('a marker from ANOTHER slot must not speak for this one', () => {
  ...
  it('re-stamps the marker on revisit so it cannot expire under its own draft', async () => {
    ...
    const findMarkerKey = () => {
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (k?.startsWith(`${PANE_RECOVERY_KEY}:page:slot-a`)) return k
      }
      throw new Error('no staged-send marker for slot-a')
    }
    const markerKey = findMarkerKey()
    const parked = JSON.parse(localStorage.getItem(markerKey) ?? 'null')
    localStorage.setItem(markerKey, JSON.stringify({ ...parked, ts: aged }))
    ...
    await waitFor(() => {
      const after = JSON.parse(localStorage.getItem(markerKey) ?? '{}')
      expect(after.ts, 'the revisit left the marker on its original stamp, so it expires first')
  ...
  it('keeps the slot-a warning when the FIRST keystroke lands after slot-b failed', async () => {
    // The containment basis was seeded from whichever payload failed LAST, so slot-a's own draft
    // was checked against slot-b's text and the very first keystroke retired slot-a's warning.
    sendChat.mockRejectedValue(new DOMException('The operation was aborted.', 'AbortError'))
    const store = makeStore(false, ['slot-b'])
    await renderPage(store)
    await sendText('slot a payload')
    ...
    await act(async () => { store.dispatch({ type: 'chat/setActiveSlot', payload: 'slot-a' }) })
    const box = () => screen.getByLabelText('Message input') as HTMLTextAreaElement
    await waitFor(() => expect(box().value).toContain('slot a payload'))
    expect(screen.queryAllByText(/send it twice/i).length,
      'premise: slot-a is captioned before the keystroke').toBeGreaterThan(0)

    // One keystroke that KEEPS slot-a's own payload, so nothing about slot-a was edited.
    await act(async () => { fireEvent.change(box(), { target: { value: 'slot a payload!' } }) })

    expect(loadStagedSend('slot-a'),
      'slot-a was never edited, so its staged marker must survive its own keystroke').toBeTruthy()
    expect(screen.queryAllByText(/send it twice/i).length,
      'and its duplicate-resend warning must survive too').toBeGreaterThan(0)
  })
})
...
describe('the warning offers no control that could erase a merged draft', () => {
  it('keeps text typed during the request and renders no discard affordance', async () => {
    ...
    expect(screen.queryByTestId('delivery-exit-discard')).toBeNull()
  })
})
...
describe('an option send never touches the composer it did not come from', () => {
  ...
  /* the composer-clear is gated on `!optionText` but the
   * stash gate was not, so a queued option send could be cancelled back INTO the intact draft. */
  it('writes no recovery record, so a cancel cannot merge the option into the draft', async () => {
    preSendStash.clear()
    ...
    expect(preSendStash.size,
      'an option send left the draft intact, so its text is not the draft to give back').toBe(0)
    // Control on the SAME harness: a typed send of the same shape does stash, so the zero above
    // measures the option gate rather than a harness that never reaches the stash.
    cleanup()
    preSendStash.clear()
    await renderPage(makeStore())
    await sendText('typed by hand')
    expect(preSendStash.size, 'control: the typed path still stashes').toBe(1)
  })
})
...
describe('an off-screen failure does not steal the active slot warning', () => {
  it('leaves the visible slot own duplicate-send warning standing', async () => {
    ...
    const visible = () => Array.from(document.querySelectorAll('span[role="status"]'))
      .filter(e => /send it twice/i.test(e.textContent ?? ''))
  ...
  /* UX at f0400eb33: retiring the row's `deliveryUnknown` on a composer edit un-dimmed a still
   * unconfirmed bubble, so the transcript vouched for a delivery nothing had proved. The STRIP is
   * what an edit retires; the row's markers wait for a receipt, an echo, or an explicit exit. */
  it('retires the composer strip on an edit past containment, but neither row marker', async () => {
    sendChat.mockRejectedValue(new DOMException('The operation was aborted.', 'AbortError'))
    const store = makeStore()
    await renderPage(store)
    await sendText('orphan candidate')

    await waitFor(() => expect(userRow(store)?.meta?.deliveryUnknown).toBe(true))
    expect(userRow(store)?.meta?.pendingServerRow,
      'the send is retained while its caption is up').toBe(true)

    const box = () => screen.getByLabelText('Message input') as HTMLTextAreaElement
    // Still contained: the strip stands, so both row markers stand with it.
    await act(async () => { fireEvent.change(box(), { target: { value: 'orphan candidate extra' } }) })
    expect(userRow(store)?.meta?.pendingServerRow).toBe(true)
    expect(userRow(store)?.meta?.deliveryUnknown).toBe(true)

    // Past containment: the strip is spent, but nothing here proves the POST never left.
    await act(async () => { fireEvent.change(box(), { target: { value: 'a different prompt' } }) })
    await waitFor(() => expect(screen.queryAllByText(/send it twice/i),
      'the strip is what an edit retires').toHaveLength(0))
    expect(userRow(store)?.meta?.deliveryUnknown,
      'the row stays dimmed until something actually proves delivery').toBe(true)
    expect(document.querySelector('[data-delivery-caption]'),
      'and keeps its caption, so the bubble never reads as delivered').not.toBeNull()
    expect(userRow(store)?.meta?.pendingServerRow,
      'retention is retired only by the row appearing in a page, or an explicit outcome').toBe(true)
  })

  /* the retirement gate exempted only an unreadable 2xx and a queued acceptance
   * with no id, so a TIMED-OUT send had its pre-send record deleted. The deadline fires before any
   * receipt, so the server is free to have queued it — and a later `queue_push` then found nothing
   * to adopt, leaving cancel with the redacted card text and no attachments. */
  it('keeps a timed-out send recoverable, because the server may still have queued it', async () => {
    preSendStash.clear()
    queuedSendStash.clear()
    sendChat.mockRejectedValue(new DOMException('The operation was aborted.', 'AbortError'))
    const store = makeStore()
    await renderPage(store)
    await sendText('a prompt worth keeping')

    await waitFor(() => expect(userRow(store)?.meta?.deliveryUnknown).toBe(true))
    const sendId = userRow(store)?.meta?.sendId as string
    expect(sendId, 'the send minted an id').toBeTruthy()

    expect(preSendStash.has(sendId),
      'a deadline settles nothing, so the lossless record must survive it').toBe(true)

    // The consequence the finding names: a queue_push arriving after the timeout must find the
    // record, so cancelling that card can still recover the typed text and its attachments.
    adoptPreSendStash(sendId, 'q-after-timeout', 'a prompt worth keeping')
    const adopted = queuedSendStash.get('q-after-timeout')
    expect(adopted, 'the queue entry adopts the surviving record').toBeTruthy()
    expect(adopted?.raw).toBe('a prompt worth keeping')
  })

  /* the pre-send stash wrote {raw, sent} for
   * EVERY send, so a knowledge send stashed composer text WITHOUT the prepended block and cancelling
   * its queued card silently dropped the selected context. Read in the adoption window, because a
   * `queue_push` reaches the stash before the receipt retires it. */
  it('keeps the injected context on a cancelled knowledge send', async () => {
    preSendStash.clear()
    queuedSendStash.clear()
    // Control FIRST, on its own page: with nothing diverging the wire text this harness DOES stash,
    // so the assertions below measure the gate rather than a harness that never reaches it.
    sendChat.mockReturnValue(new Promise(() => {}))
    await renderPage(makeStore())
    await sendText('plain words')
    expect(preSendStash.size, 'the lossless path still stashes').toBe(1)
    cleanup()
    preSendStash.clear()
    ...
    const store = makeStore()
    await renderPage(store)
    // Hang the POST so the receipt -- and the retirement it performs -- never runs.
    sendChat.mockReturnValue(new Promise(() => {}))
    await sendText('summarise this')

    const posted = String(sendChat.mock.calls.at(-1)?.[0] ?? '')
    expect(posted, 'the knowledge block really did reach the wire').toContain('[KNOWLEDGE CONTEXT')
    const sendId = userRow(store)?.meta?.sendId as string
    expect(sendId).toBeTruthy()

    // What the WS does on `queue_push`, then what a cancel would restore from it: the stash is
    // consumed only when its `sent` matches the card (useQueuedMessageActions).
    adoptPreSendStash(sendId, 'q-knowledge', posted)
    const stashed = queuedSendStash.get('q-knowledge')
    const restored = stashed && stashed.sent === posted ? stashed.raw : posted
    expect(restored, 'a cancel must not hand back the prompt minus its context').toContain('[KNOWLEDGE CONTEXT')
  })

  /* The other half of the same defect: the queued arm retired the pre-send record as soon as its receipt
   * carried a queue id, so the `queue_push` that arrives later had nothing to adopt. */
  it('keeps the pre-send record while a queued acceptance still awaits its push', async () => {
    preSendStash.clear()
    queuedSendStash.clear()
    sendChat.mockResolvedValue({ ok: true, json: () => Promise.resolve({ ok: true, queued: true, queue_id: 'q-await' }) })
    const store = makeStore()
    await renderPage(store)
    await sendText('behind a busy turn')

    await waitFor(() => expect(sendChat).toHaveBeenCalled())
    const sendId = userRow(store)?.meta?.sendId as string
    expect(sendId).toBeTruthy()
    expect(preSendStash.has(sendId),
      'nothing else can hand the raw payload to the push that has not arrived yet').toBe(true)
  })

  /* UX at 7e65ece1a: once the caption swaps to "Message delivered", the affordance still read
   * "Discard unsent message" -- the same state told the reader the message both was and was not
   * sent, and they had no way to tell which claim to trust. */
  it('stops calling a delivered message unsent once the send is confirmed', async () => {
    sendChat.mockRejectedValue(new DOMException('The operation was aborted.', 'AbortError'))
    const store = makeStore()
    await renderPage(store)
    await sendText('timed out, then confirmed')

    await waitFor(() => expect(userRow(store)?.meta?.deliveryUnknown).toBe(true))
    // Premise: while delivery is unknown the message really is unsent, so that label is right.
    expect(await screen.findByText(/^Discard message\b.*/i)).not.toBeNull()

    const sendId = userRow(store)?.meta?.sendId as string
    await act(async () => { store.dispatch(confirmOptimisticSend({ slot: 'slot-a', sendId })) })

    await waitFor(() => expect(screen.queryByText(/^Discard message\b.*/i),
      'a delivered message must not be offered a control calling it unsent').toBeNull())
    expect(screen.getByText(/^Clear the composer$/i),
      'the affordance stays -- only its claim about delivery changes').not.toBeNull()
  })

  /* UX at 6f2d77d55: a revisit re-arms the caption deliberately WITHOUT a Discard seed, because
   * the persisted draft is the whole composer and this path cannot tell how much of it the send
   * carried. That left a duplicate-send warning whose only release was editing the text. */
  it('offers a way out of a caption re-armed on revisit, without touching the draft', async () => {
    sendChat.mockRejectedValue(new DOMException('The operation was aborted.', 'AbortError'))
    const store = makeStore(false, ['slot-b'])
    await renderPage(store)
    await sendText('text that must survive the dismiss')
    await waitFor(() => expect(loadStagedSend('slot-a')).toBeTruthy())
    // Premise: while the payload is still in memory the destructive exit is the right one.
    expect(await screen.findByTestId('delivery-exit-discard')).not.toBeNull()

    // RELOAD, not a slot switch: the in-memory payload dies with the tab while the marker and the
    // draft persist, which is the state whose caption re-arms with nothing to discard.
    cleanup()
    await renderPage(makeStore(false, ['slot-b']))

    await waitFor(() => expect(screen.queryAllByText(/resending may send it twice/i).length).toBeGreaterThan(0))
    expect(screen.queryByTestId('delivery-exit-discard'),
      'a revisit seeds no payload, so the destructive exit stays withheld').toBeNull()

    const before = (screen.getByLabelText('Message input') as HTMLTextAreaElement).value
    await act(async () => { fireEvent.click(screen.getByTestId('delivery-exit-dismiss')) })
    await waitFor(() => expect(screen.queryAllByText(/resending may send it twice/i)).toHaveLength(0))
    expect((screen.getByLabelText('Message input') as HTMLTextAreaElement).value,
      'dismiss acknowledges the warning; it must not delete what the user may still want').toBe(before)
  })
