...
describe('FollowUpBar', () => {
  ...
  describe('recommendation badge', () => {
    ...
    it('gives the badge a fill the PICKED chip body does not share, so it stays legible after the pick', () => {
      const { container } = render(
        <FollowUpBar options={['Merge it now']} recommended={'Merge it now'} picked={new Set(['Merge it now'])} onSelect={() => {}} />,
      )
      const bgTokens = (el: Element) => el.className.split(/\s+/).filter(c => c.startsWith('bg-'))
      ...
      expect(bgTokens(chip)).toEqual(['bg-accent-subtle'])
      expect(bgTokens(badge!)).toHaveLength(1)
      expect(bgTokens(badge!)).not.toEqual(bgTokens(chip))
    })
