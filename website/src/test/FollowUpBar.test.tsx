...
describe('FollowUpBar', () => {
  ...
  describe('long option labels', () => {
    ...
    it('carries the full label on a BADGED chip too, where the badge takes row width', () => {
      ...
      // The recovery is the tooltip, which carries the whole option text; the native `title`
      // it replaced is gone, so reading that attribute tested nothing.
      fireEvent.focus(container.querySelector('button')!)
      expect(screen.getByRole('tooltip').textContent ?? '').toContain(option)
    })
  ...
  //
  // The point of the badge is its POSITION: it sits OUTSIDE the one clamped
  // line, so no label length can hide it. These assertions check it is a
  // SIBLING of the clamped span and never a descendant — an assertion on the
  // text alone would pass either way and prove nothing.
  describe('recommendation badge', () => {
    ...
    it('a duplicate label is reachable from an options line, so the case is real', () => {
      expect(parseOptions('<!-- recommended:1 -->\n[OPTIONS: A | A]').options).toEqual(['A', 'A'])
    })
    ...
    it('badges a prototype-keyed option parsed from a real marker', () => {
      const { options, recommended } = parseOptions('<!-- recommended:1 -->\n[OPTIONS: __proto__ | Safe]')
