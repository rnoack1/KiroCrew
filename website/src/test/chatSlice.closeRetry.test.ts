...
/** A slot row. `incarnation` is the LIVE-object identity a resume does not carry over,
 *  so a distinct one is what marks a replacement resumed under a reused key. */
const slot = (key: string, incarnation = `inc-${key}`): ChatSlot =>
...
describe('close confirms after retiring the tombstone', () => {
  ...
  it('recovers when the confirming read fails', async () => {
    ...
    mockSlots.mockResolvedValueOnce([slot('chat-1'), slot('chat-2')])
      ...
      // A REPLACEMENT resumed under the reused key carries its own stamp.
      .mockResolvedValue([slot('chat-1'), slot('chat-2', 'inc-resumed')])
    const s = store()
    await s.dispatch(deleteSlot('chat-2') as never)
    for (let i = 0; i < 6; i++) await new Promise(r => setTimeout(r, 0))
    // A failure releases nothing, so the reads must continue.
    expect(mockSlots.mock.calls.length).toBeGreaterThanOrEqual(2)
    await vi.waitFor(() => expect(closingOf(s)['chat-2']).toBeUndefined(), { timeout: 3000 })
  })
  ...
  it('recovers when the confirming read is refused rather than failed', async () => {
    ...
    // A refusal releases nothing, so the retry arrives on the capped backoff rather than
    // in the same microtask run.
...
describe('deleteSlot close resilience', () => {
  ...
  it.each([
    ...
    ['a 5xx', status(503)],
  ])('holds the row withheld until the outcome is proven when unknown — %s', async (_l, err) => {
    mockDelete.mockRejectedValue(err)
    // The server still lists it, which is NOT proof the close failed: a lost DELETE
    // response leaves `close_slot` running, so the reply may simply have outrun it.
    mockSlots.mockResolvedValue([slot('chat-1'), slot('chat-2')])
    const s = store()
    await expect(s.dispatch(deleteSlot('chat-2')).unwrap()).rejects.toBeDefined()
    expect(mockSlots).toHaveBeenCalled()
    // Withheld, and the tombstone survives so a later read can settle it.
    await vi.waitFor(() => expect(keysOf(s)).toEqual(['chat-1']))
    expect(closingOf(s)['chat-2']).toBeDefined()
  })

  /** The other half of the same contract: an accepted read that OMITS the key proves
   *  the pop, retires the tombstone, and leaves the row gone for good. */
  it('retires the tombstone once a read omits the key', async () => {
  ...
  it('releases the tombstone when the retirement read fails, so a resume stays visible', async () => {
    mockDelete.mockResolvedValue({ ok: true })
    // Every read fails, so nothing ever proves anything: the row stays withheld rather
    // than being revealed while the close may still be running.
    mockSlots.mockRejectedValue(new Error('gateway down'))
    const s = store()
    await s.dispatch(deleteSlot('chat-2')).unwrap()
    await new Promise(r => setTimeout(r, 400))
    expect(closingOf(s)['chat-2']).toBeDefined()
    // Once a read APPLIES and dates the key as a new instance, the resume is visible.
    mockSlots.mockResolvedValue([slot('chat-1'), slot('chat-2', 'inc-resumed')])
    await vi.waitFor(() => expect(closingOf(s)['chat-2']).toBeUndefined(), { timeout: 3000 })
  })
  ...
  it('keeps re-reading until one authoritative list lands', async () => {
    ...
    mockSlots
      ...
      // The list that finally lands dates the key as a NEW instance -- a real resume.
      .mockResolvedValue([slot('chat-1'), slot('chat-2', 'inc-resumed')])
    const s = store()
    await s.dispatch(deleteSlot('chat-2')).unwrap()
    await vi.waitFor(() => expect(keysOf(s)).toEqual(['chat-1', 'chat-2']), { timeout: 4000 })
  ...
  it('does not give up after the old attempt cap', async () => {
    ...
    mockSlots
      ...
      // The list that finally lands dates the key as a NEW instance -- a real resume, which
      // is the one listed shape that releases the hold.
      .mockResolvedValue([slot('chat-1'), slot('chat-2', 'inc-resumed')])
    const s = store()
    await s.dispatch(deleteSlot('chat-2') as never)
