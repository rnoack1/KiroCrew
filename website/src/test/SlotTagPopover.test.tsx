...
describe('SlotTagPopover (gateway offline)', () => {
  ...
  it('clears the new-tag field once the name it sent is created', async () => {
    renderPopover({ slotKey: 'chat-1-100', slots: [{ key: 'chat-1-100', tags: [] }], connected: true })
    const field = await screen.findByPlaceholderText(/new tag/i)
  ...
  it('leaves a newer draft alone when the earlier create lands', async () => {
    renderPopover({ slotKey: 'chat-1-100', slots: [{ key: 'chat-1-100', tags: [] }], connected: true })
    const field = await screen.findByPlaceholderText(/new tag/i)
    ;(field as HTMLInputElement).focus()
    fireEvent.change(field, { target: { value: 'ops' } })
    fireEvent.keyDown(field, { key: 'Enter' })
    // Typed before the create settles: that write has no claim on this text.
    fireEvent.change(field, { target: { value: 'platform' } })
    await waitFor(() => expect(api.createChatTag).toHaveBeenCalledWith('ops'))
    await act(async () => { await Promise.resolve() })
    expect((field as HTMLInputElement).value).toBe('platform')
  })
})
