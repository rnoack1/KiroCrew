 ...
 */
import { __resetClosingIntentForTests, __setClosingAckWindowForTests } from '../utils/slotClosingIntent'
...
beforeEach(() => {
  ...
  localStorage.clear()
  // The close now runs a closing handshake; a leftover intent or veto from a prior test
  // decides the next one, and the real ack window would be spent on every close here.
  __resetClosingIntentForTests()
  __setClosingAckWindowForTests(0)
  seamLabel = ''
  vi.clearAllMocks()
...
afterEach(async () => {
  vi.useRealTimers()
  // The close spans the closing-handshake await, so one started in this test can otherwise
  // finish inside the NEXT one and land its breadcrumb in that test's mock counts.
...
describe.each(HOSTS)('$name — zero-turn option action dispatch', (host) => {
  ...
  it('a REFUSED close writes no second breadcrumb and claims nothing', async () => {
    ...
    expect(api.chatSlotNote).toHaveBeenCalledTimes(1)
    // Bans a row claiming SUCCESS; the honest failure report it used to catch as
    // well is pinned positively by `DeleteFailureSurfaced`.
    expect(errorRows(store, slot).some(m => /^closed|session closed|tab closed|successfully clos/i.test(m.content ?? ''))).toBe(false)
