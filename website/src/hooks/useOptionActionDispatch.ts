...
export function useOptionActionDispatch(params: {
  ...
  const [crossWindow, setCrossWindow] = useState(false)
  const crossWindowRef = useRef(false)
  crossWindowRef.current = crossWindow
  useEffect(() => {
    const sample = () => {
      const slot = resolveSlotRef.current()
      setCrossWindow(slot ? slotUnsentWorkSource(slot) !== null : false)
    }
  ...
  const dispatchFollowUpAction = useCallback(async (action: OptionAction, sourceKeyAtClick?: string | null) => {
    ...
    if (unsentAt !== null && (unsentAt === 'here' || claimsAreReadable())) {
      ...
      const disclosed = slotHasUnsentWorkHere(slot) || crossWindowRef.current
      // Learn it now, so the chip greys with its reason from here on.
      if (unsentAt !== 'here') setCrossWindow(true)
   ...
   */
  const slotBlocksAction = (): boolean => {
    const slot = resolveSlotRef.current()
    if (!slot) return false
    return slotHasUnsentWorkHere(slot) || crossWindow
  }
