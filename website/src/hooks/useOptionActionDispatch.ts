...
import { claimsAreReadable } from '../utils/slotDirtyBeacon'
import { slotHasUnsentWorkHere, slotUnsentWorkSource, type UnsentWorkSource } from '../utils/slotComposerRegistry'
...
export function useOptionActionDispatch(params: {
   ...
   */
  /**
   * Where a FAILED close is reported, when the host owns a durable surface.
   *
   * A transcript row is not one on every host: the close's own recovery re-fetch
   * replaces the slot's messages, taking an error row with it. A host rendering
   * `ErrorNotice` passes its setter here; without one the row is the fallback.
   */
  ...
  // Same reason as the two above: the callback is built at click time, and reading
  // `params` inside it would capture the whole object as a dependency.
  const onCloseErrorRef = useRef(params.onCloseError)
  onCloseErrorRef.current = params.onCloseError

  // Event-sampled, never on mount or a timer: the scan enumerates storage keys, and callers
  // assert neither a render nor an idle session touches storage. The dispatch closes the gap.
  const [crossWindow, setCrossWindow] = useState<UnsentWorkSource | null>(null)
  const crossWindowRef = useRef<UnsentWorkSource | null>(null)
  crossWindowRef.current = crossWindow
  useEffect(() => {
    const sample = () => {
      const slot = resolveSlotRef.current()
      setCrossWindow(slot ? slotUnsentWorkSource(slot) : null)
    }
    window.addEventListener('storage', sample)
    window.addEventListener('focus', sample)
    return () => {
      window.removeEventListener('storage', sample)
  ...
  // FALSE, unconditionally: the shared draft store is last-write-wins over one key, so no
  // host can honestly claim another window recovers its drafts. Take an argument when one can.
  ...
  const dispatchFollowUpAction = useCallback(async (action: OptionAction, sourceKeyAtClick?: string | null) => {
    ...
    // A REFUSAL needs a knowable answer, and the claim store fail-closes to "dirty" when it
    // cannot be read; `'here'` is exempt because the registry answers it without storage.
    if (unsentAt !== null && (unsentAt === 'here' || claimsAreReadable())) {
      // Did the PAINT disclose this? `crossWindow` is event-sampled, so a window that mounted
      // after another published its claim paints a live chip and the click refuses anyway.
      const disclosed = slotHasUnsentWorkHere(slot) || crossWindowRef.current !== null
      // Learn it now, so the chip greys with its reason from here on.
      if (unsentAt !== 'here') setCrossWindow(unsentAt)
      // A row only for a block the user could SEE. Writing one for a click this component
      // invited leaves a permanent transcript record of its own paint being wrong.
      if (disclosed) notify(i18nT(unsentNoticeKey(unsentAt)))
      return
    }
    ...
    try {
      await closeSessionWithConfirm(slot, {
        ...
        forceConfirm: true,
        // This path owns an error channel already, so the failure lands as a row in
        // the session that survived rather than through the hook's own fallback.
        onError: (message: string) => { const sink = onCloseErrorRef.current; if (sink) sink(message); else notify(message) },
        ...
        beforeDelete: async () => {
          ...
          if (lateUnsentAt !== null && (lateUnsentAt === 'here' || claimsAreReadable())) {
  ...
  /**
   * Does the SLOT block the action right now, as far as a render may ask?
   *
   * The chip used to paint from the host's own composer alone, while the click asked about the
   * whole slot, so a chip in full danger paint refused on click and left a permanent error row.
   * The cross-window tiers enumerate storage keys, which a render path must not do — it costs a
   * scan per paint and breaks callers that assert no storage access — so the answer for those
   * comes from `crossWindow`, refreshed off-render, and paint stays a plain read. That answer is
   * event-sampled, so it can still LAG a claim published before this window mounted; the
   * dispatch handles that case by greying the chip rather than recording a row.
   */
  const slotBlocksAction = (): UnsentWorkSource | null => {
    const slot = resolveSlotRef.current()
    if (!slot) return null
    return slotHasUnsentWorkHere(slot) ? 'here' : crossWindow
  }

  return { dispatchFollowUpAction, slotBlocksAction }
}
