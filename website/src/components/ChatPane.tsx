...
export default function ChatPane({
  ...
  const strandedSends = useRef(new Map<string, { text: string; files: string[]; sendId?: string }>())
  ...
  const onComposerInput = useCallback((v: string) => {
    ...
    // The marker retires on edit as ChatPage's does, but the persisted TEXT does not: the mirror
    // effect below rewrites it, so an edit updates the copy rather than removing it.
  ...
  const restoreIntoComposer = useCallback((text: string, files: string[] = [], sendId?: string) => {
    ...
    setRecoveredPayload({ text: mergedText, files: mergedFiles })
    persistRecovery(slotKey, { text: mergedText, files: mergedFiles, ...(sendId ? { sendId } : {}) })
  ...
  useEffect(() => {
    ...
    // Seeded here too, or a reload re-arms the "may send it twice" caption with no Discard beside
    // it and the user has to empty the composer by hand.
    setRecoveredPayload({ text: parked.text, files: parked.files })
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const gen = (loadPaneRecoveries()[slot]?.gen ?? 0) + 1
      persistRecovery(slot, { text: input, files: pendingFiles, gen, ...(armed.sendId ? { sendId: armed.sendId } : {}) })
  ...
  const doSend = useCallback((optionText?: string) => {
    ...
    const consumedGen = loadPaneRecoveries()[slotKey]?.gen
    if (!optionText) preSendStash.set(sendId, { raw: text, files, sent: llm })
    void sendTurn({ message: llm, slot: slotKey, meta }).then((receipt) => {
      ...
      // A definitive receipt retires the payload it CARRIED, matched by generation: a draft the
      // user typed while this POST was open is a newer generation and outlives the receipt.
      if (receipt.status === 'queued' || receipt.status === 'dispatched') {
        if ((loadPaneRecoveries()[slotKey]?.gen ?? 0) === (consumedGen ?? 0)) {
          recoveryArmed.current = null
