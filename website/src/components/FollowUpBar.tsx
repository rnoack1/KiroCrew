...
function sendRefused(option: string): boolean {
  return dispatchIsCommandShaped(option)
}
...
function FollowUpBar({ options, recommended, picked, onSelect, onSend, quickSend, layout = 'multiline', sourceKey }: FollowUpBarProps) {
  ...
  // sr-only note reaches no sighted one -- yet this chip behaves differently from its
  // siblings, and the dashed edge alone does not say why.
  //
  // ONE note, because there is one case. The recommendation used to be a prefix inside the
  // label, so a chip could also be "marked but not strippable" and needed its own note (plus
  // a third for a menu carrying both kinds). Out of band there is no such chip: a label is
  // never rewritten, so the only chip that behaves differently is one that would dispatch a
  // command.
  ...
  if (!commandShaped) return bar
  return (
    <>
      {bar}
      <p data-testid="command-shaped-note" className="mt-1 px-1 text-[11px] leading-tight text-text">
        {i18nT('components.followUpBar.command_shaped_kept')}
      </p>
    </>
  )
}
