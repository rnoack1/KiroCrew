  ...
  if (variant === 'inline') {
    return (
      ...
      >
        ...
        <span className={`min-w-0 ${messageClassName}`} style={{ overflowWrap: 'anywhere' }}>{withOriginLink(message)}</span>
        {askAgent && (
          <AskAgentButton
            report={report}
            message={message}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1 whitespace-pre-wrap" style={{ overflowWrap: 'anywhere' }}>
        ...
        {messageClassName
          ? <span className={messageClassName}>{withOriginLink(message)}</span>
