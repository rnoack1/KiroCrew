...
import chatReducer, { retainedSend, switchSlot, refreshSlot, resumeFromHistory, setActiveSlot, appendMessage, appendSlotMessage, warmSlotCache, clearPendingServerRow, dropUnconfirmedRow, confirmOptimisticSend, markDeliveryUnknown, sseChatMessage, appendQueuedMessage } from './chatSlice'
import { api } from '../api/client'
import { queuedSendStash, stashQueuedSend, restashEditedSend } from '../utils/queuedSendStash'
...
describe('a failed send stops being re-attachable', () => {
  ...
  /* reconciliation DELETES `sendId` and rewrites it as
  ...
  it('retires the predecessor WARNING but not its retention, which only an outcome clears', async () => {
    // A later send confirming says nothing about this one, which may have persisted with its response
    // lost. So the caption goes, retention stays, and the user's own exit is what drops the row.
    const store = makeStore()
    store.dispatch(setActiveSlot(SLOT))
    // Real history the server does have, so the unsent attempt is not simply above the page.
    store.dispatch(appendMessage({
      role: 'assistant', content: 'earlier reply', cls: '', ts: '2026-08-29T17:00:00.000Z',
      meta: { mid: 'm-earlier' },
    }))
    store.dispatch(appendMessage({
      role: 'user', content: 'hello', cls: '', ts: '2026-08-29T17:54:01.900Z',
      meta: retainedSend({ sendId: SEND_ID }),
    }))
    store.dispatch(markDeliveryUnknown({ slot: SLOT, sendId: SEND_ID }))
    const prior = () => store.getState().chat.messages.find(m => m.content === 'hello')
    expect(prior()?.meta?.deliveryUnknown,
      'premise: the first attempt is unreceipted').toBe(true)
    expect(prior()?.meta?.pendingServerRow, 'premise: and it is armed retained').toBe(true)

    const RESENT = 's-resent-0001'
    store.dispatch(appendMessage({
      role: 'user', content: 'the same words again', cls: '',
      ts: '2026-09-07T19:59:00.000Z',
      meta: retainedSend({ sendId: RESENT, optimistic: true }),
    }))
    // The server echoes the resend, which reconciles it and demotes the prior doubt.
    store.dispatch(sseChatMessage({
      slot: SLOT, role: 'user', content: 'the same words again', cls: '',
      ts: '2026-09-07T20:00:00.000Z',
      meta: { sendId: RESENT, mid: 'm-resent-1' },
    }))

    expect(prior()?.meta?.deliveryUnknown,
      'a later send proves nothing about this one, so its doubt must survive').toBe(true)
    expect(prior()?.meta?.pendingServerRow,
      'retention survives: a later receipt is no proof THIS send never landed').toBe(true)

    // The explicit outcome retention waits for. Nothing proves this row landed, so it may go.
    store.dispatch(dropUnconfirmedRow({ slot: SLOT, sendId: SEND_ID }))
    expect(userRows(store).map(m => m.content),
      'the user\u2019s own exit is what drops it').toEqual(['the same words again'])

  })

  it('refuses to drop a row the server has acknowledged', async () => {
    // Positive control: dropping on the caller alone would delete a DELIVERED message, which is the
    // loss retention exists to prevent. A server `mid` is proof the transcript owns the row.
    const store = storeWithOptimisticBubble()
    store.dispatch(confirmOptimisticSend({ slot: SLOT, sendId: SEND_ID, mid: 'm-server-1' }))

    store.dispatch(dropUnconfirmedRow({ slot: SLOT, sendId: SEND_ID }))

    expect(userRows(store), 'an acknowledged row is not the user\u2019s to drop').toHaveLength(1)
  })

  it('keeps retaining a send whose doubt was never demoted', async () => {
    // Positive control: retiring retention unconditionally would satisfy the case above while
    // dropping the very bubble this store exists to preserve. No later echo arrives here.
    const store = storeWithOptimisticBubble()
    store.dispatch(markDeliveryUnknown({ slot: SLOT, sendId: SEND_ID }))
    detail.mockResolvedValue(page([]))
    ...
    expect(store.getState().chat.messages[0]?.meta?.pendingServerRow,
      'an unresolved send keeps its retention').toBe(true)
    expect(userRows(store)).toHaveLength(1)
  })
})
...
describe('a later confirmed turn leaves an earlier unconfirmed caption alone', () => {
  it('keeps deliveryUnknown on the earlier row, which nothing here saw land', () => {
    const store = makeStore()
    store.dispatch(setActiveSlot(SLOT))
    ...
    const rows = store.getState().chat.messages.filter(m => m.role === 'user')
    // Clearing it here made an unsent prompt render as an ordinary delivered row, with no cue left
    // and no path back to the doubt. Only this row's OWN confirmation may retire it.
    expect(rows[0].meta?.deliveryUnknown,
      'a send that never left must keep saying so').toBe(true)
    expect(rows[0].meta?.deliveryConfirmed,
      'and it is certainly not upgraded to confirmed').toBeUndefined()
    // Negative control: the row that WAS confirmed carries its own proof.
    expect(rows[1].meta?.deliveryConfirmed).toBe(true)
  })

  it('leaves it alone on the RECEIPT path too, not just the WS echo', () => {
    const store = makeStore()
    store.dispatch(setActiveSlot(SLOT))
    store.dispatch(appendMessage({ role: 'user', content: 'never left', cls: '', ts: '2026-08-29T17:00:00.000Z', meta: retainedSend({ sendId: 's-old-1' }) }))
    store.dispatch(markDeliveryUnknown({ slot: SLOT, sendId: 's-old-1' }))
    ...
    const rows = store.getState().chat.messages.filter(m => m.role === 'user')
    expect(rows[0].meta?.deliveryUnknown,
      'the receipt path must not settle a send it says nothing about').toBe(true)
    expect(rows[0].meta?.deliveryConfirmed).toBeUndefined()
    // Negative control: the receipt-carrying row keeps its proof.
    expect(rows[1].meta?.deliveryConfirmed).toBe(true)
  })
})
...
/* the retirement scan was bounded to the last 50 rows, so a discard the
...
describe('a missed queue_push must not lose queued-send recovery', () => {
...
describe('resumeFromHistory.fulfilled keeps an unconfirmed user bubble', () => {
  // First Principles at 6cf1ede6e: three of the four fetched-page rebuilds of `state.messages` run
  // through the shared owner; this one merged pastes only, so it dropped the local-only row.
  beforeEach(() => vi.clearAllMocks())

  /** `ok: true` plus a chat-page surface is what reaches the rebuild; without it the handler
   *  early-returns and the test passes while measuring nothing. */
  const resume = (store: ReturnType<typeof makeStore>, messages: unknown[]) => {
    const arg = { key: SLOT, title: SLOT } as never
    store.dispatch(resumeFromHistory.fulfilled(
      { ok: true, key: SLOT, messages, hasMore: false, nextBefore: 0, total: messages.length } as never,
      'req-resume-1', arg,
    ))
    // Premise: `ok` absent (or a non-chat surface) early-returns before the rebuild, which would
    // make every assertion below pass without the path under test ever running.
    expect(store.getState().chat.activeSlot, 'the rebuild arm must have run').toBe(SLOT)
  }

  it('does not drop the bubble when the resumed history predates the server append', () => {
    const store = storeWithOptimisticBubble()
    resume(store, [])

    const rows = userRows(store)
    expect(rows, 'the just-sent bubble must survive a resume that cannot know about it').toHaveLength(1)
    expect(rows[0].content).toBe('hello')
    expect(rows[0].meta?.sendId).toBe(SEND_ID)
  })

  it('does not duplicate the bubble once the resumed history contains the row', () => {
    // Positive control: re-adding unconditionally would satisfy the assertion above and ship a
    // double bubble on the far commoner path where the server row IS present.
    const store = storeWithOptimisticBubble()
    resume(store, [serverUserRow])

    expect(userRows(store), 'the server row and the local row are the same send').toHaveLength(1)
  })

  it('does not carry the bubble into a DIFFERENT session being resumed', () => {
    // `state.messages` still belonged to the slot being LEFT, so the retained send was re-seated
    // into the resumed transcript. The same-session test above controls for over-correcting.
    const store = storeWithOptimisticBubble()
    const other = FRESH_SLOT
    store.dispatch(resumeFromHistory.fulfilled(
      { ok: true, key: other, messages: [], hasMore: false, nextBefore: 0, total: 0 } as never,
      'req-resume-2', { key: other, title: other } as never,
    ))
    expect(store.getState().chat.activeSlot, 'premise: the rebuild arm must have run').toBe(other)

    expect(userRows(store),
      'the send authored in another session must not follow the user here').toHaveLength(0)
  })
})

/* hydration dropped the server's `edited` marker, so a refetch after a
 * REDACTION-EQUIVALENT edit kept both the carried `rawSend` and the queue-id record. The shape check
 * (`rawSend.sent !== content`) cannot see that edit, because the display text did not move. */
describe('an EDITED queue entry leaves no pre-edit recovery state behind', () => {
  const ATT = '/tmp/Q3 report.pdf'
  const DISPLAY = `token=[redacted]\n[attached_file 1] ${ATT}`
  const PRE_EDIT = `token=OLD-SECRET\n[attached_file 1] ${ATT}`

  it('retires the queue-id record and drops the carried rawSend', async () => {
    stashQueuedSend('q9', { raw: PRE_EDIT, files: [ATT], sent: DISPLAY })
    const store = makeStore()
    store.dispatch(setActiveSlot(SLOT))
    // `rawSend.sent` MATCHES the display text, so only the marker can condemn it.
    store.dispatch(appendMessage({
      role: 'queued', content: DISPLAY, cls: '', ts: '2026-09-09T15:00:00.000Z',
      meta: { queueId: 'q9', rawSend: { text: PRE_EDIT, files: [ATT], sent: DISPLAY } },
    } as never))
    expect(store.getState().chat.messages.find(m => m.meta?.queueId === 'q9')?.meta?.rawSend,
      'premise: the pre-edit payload is on the card').toBeDefined()

    detail.mockResolvedValueOnce({
      ...page([], false), queue: [{ id: 'q9', content: DISPLAY, edited: true }],
    } as never)
    await store.dispatch(refreshSlot(SLOT) as never)

    const row = store.getState().chat.messages.find(m => m.meta?.queueId === 'q9')
    expect(row, 'premise: the card survived the refetch').toBeDefined()
    expect(row?.meta?.rawSend,
      'an edited entry must not keep the payload it was SENT with').toBeUndefined()
    expect(queuedSendStash.get('q9'),
      'nor the queue-id record written before that edit').toBeUndefined()
  })

  it('keeps both when the entry is NOT edited', () => {
    // Control: retiring unconditionally would satisfy the assertions above while destroying the
    // recovery this stash exists for.
    stashQueuedSend('q8', { raw: PRE_EDIT, files: [ATT], sent: DISPLAY })
    expect(queuedSendStash.get('q8')?.files).toEqual([ATT])
  })
})

/* two refetch paths discarded live local state -- the same class of defect as
 * this PR's subject. The server's `edited` flag is permanent, so a blind retire dropped the record a
 * settle had just written; and hydration carried no `editPending`, so an in-flight edit vanished. */
describe('a refetch keeps live queue-edit state', () => {
  const ATT = '/tmp/Q3 report.pdf'
  const PRE = `before\n[attached_file 1] ${ATT}`
  const SUBMITTED = `after MY edit\n[attached_file 1] ${ATT}`
  const DISPLAY = `after [redacted]\n[attached_file 1] ${ATT}`

  const servedQueue = (q: unknown) => detail.mockResolvedValueOnce(
    { ...page([], false), queue: [q] } as never)

  it('keeps the record the CURRENT edit settled, though `edited` stays set forever', async () => {
    stashQueuedSend('q7', { raw: PRE, files: [ATT], sent: PRE })
    // A settle, exactly as the success and echo paths perform it.
    restashEditedSend('q7', SUBMITTED, DISPLAY)
    const store = makeStore()
    store.dispatch(setActiveSlot(SLOT))

    servedQueue({ id: 'q7', content: DISPLAY, edited: true })
    await store.dispatch(refreshSlot(SLOT) as never)

    expect(queuedSendStash.get('q7')?.raw,
      'the record describes the entry as it now stands, so it is not stale').toBe(SUBMITTED)
    expect(queuedSendStash.get('q7')?.files,
      'and it is still the only holder of the spaced path').toEqual([ATT])
  })

  it('still retires a record that PREDATES the edit', async () => {
    // Control: sparing every edited entry would satisfy the test above while restoring pre-edit text.
    stashQueuedSend('q6', { raw: PRE, files: [ATT], sent: PRE })
    const store = makeStore()
    store.dispatch(setActiveSlot(SLOT))

    servedQueue({ id: 'q6', content: DISPLAY, edited: true })
    await store.dispatch(refreshSlot(SLOT) as never)

    expect(queuedSendStash.get('q6'),
      'nothing settled this record for the edited text').toBeUndefined()
  })

  it('keeps an UNSETTLED edit and its optimistic text through a refetch', async () => {
    const store = makeStore()
    store.dispatch(setActiveSlot(SLOT))
    store.dispatch(appendMessage({
      role: 'queued', content: SUBMITTED, cls: '', ts: '2026-09-09T18:00:00.000Z',
      meta: { queueId: 'q5', editPending: SUBMITTED },
    } as never))

    // The page was read BEFORE the edit landed, so it still carries the pre-edit text.
    servedQueue({ id: 'q5', content: PRE })
    await store.dispatch(refreshSlot(SLOT) as never)

    const row = store.getState().chat.messages.find(m => m.meta?.queueId === 'q5')
    expect(row?.meta?.editPending,
      'the response guard has nothing to match on without it').toBe(SUBMITTED)
    expect(row?.content, 'and the card must not revert under the user').toBe(SUBMITTED)
  })
})
