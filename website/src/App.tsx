...
export default function App() {
  ...
  const [bootSlotsRetrying, setBootSlotsRetrying] = useState(false)
  const [bootSlotsRetryCount, setBootSlotsRetryCount] = useState(0)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    let lastOrder = order()
    noteLiveSessionInstances(last, lastOrder)
    return store.subscribe(() => {
      const next = store.getState().dashboard.slots
      const nextOrder = order()
      // An identical frame preserves the ARRAY reference, so the reference alone cannot see a
      // new emission, and a registry left behind dates the session's own writes as stale.
      const orderMoved =
        nextOrder.generation !== lastOrder.generation || nextOrder.epoch !== lastOrder.epoch
      if (next === last && !orderMoved) return
      last = next
      lastOrder = nextOrder
      noteLiveSessionInstances(next, nextOrder)
    })
  }, [])
            ...
            <div className="animate-rise rounded-lg bg-bg-elevated shadow-xl overflow-hidden">
              ...
              <ErrorNotice
                ...
                action={(
                  <button
                    type="button"
                    className="underline text-text bg-transparent border-none cursor-pointer p-0 disabled:opacity-60 disabled:cursor-default"
                    onClick={() => {
                      setBootSlotsRetrying(true)
                      void refetchBootSlots().finally(() => {
                        setBootSlotsRetrying(false)
                        setBootSlotsRetryCount(n => n + 1)
                      })
                    }}
                    disabled={bootSlotsRetrying}
                    data-testid="boot-slots-retry"
                  >
                    {bootSlotsRetrying
                      ? i18nT('app.boot_slots_retrying')
                      : bootSlotsRetryCount > 0
                        ? i18nT('app.boot_slots_retry_again')
