...
class TestReuseRefusesAMovedProject:
    ...
    async def test_a_cancelled_stale_eviction_does_not_strand_the_permit(self, cfg):
        ...
        assert not stranded.semaphore.locked(), (
            "a cancellation while the eviction waits for the registry lock must not leave "
            "the permit held -- nothing else can release it and the key is locked for good"
        )
    ...
    async def test_a_newer_change_supersedes_the_earlier_arm(self, cfg):
        ...
        first = boundary._live_arm(folded)
        ...
        assert boundary._live_arm(folded) == str(Path("/projects/gamma")), (
    ...
    async def test_a_temporary_removal_keeps_both_the_arm_and_the_generation(self, cfg):
        ...
        assert boundary._live_arm(folded) == self.BETA, (
    ...
    async def test_the_claim_gate_reads_the_providers_own_workspace_default(self, cfg):
        ...
        resolved = mgr._allocation_boundary()._resolved_cwd("")
    ...
    async def test_a_live_session_bound_to_another_project_is_not_reused(self, cfg):
        ...
        assert seen[0].cwd == "/projects/alpha", "precondition: the first bind was recorded"
        seen[0].shutdown.assert_awaited_once()
        assert seen[-1].cwd == "/projects/beta", "the replacement binds at the requested dir"
        await mgr.close_all()
