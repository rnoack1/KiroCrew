...
class TestTheObserverBroadcastIsScrubbed:
    ...
    def test_the_observer_scrubs_the_message_before_broadcasting(self) -> None:
        ...
        block = src[start:end]
        assert (
            'scrub_loop_text(loop.message, field="message")' in src[:start][-1200:]
        ), "the message is not scrubbed before the broadcast"
        assert '"message": safe_message' in block, "the payload still ships the raw message"
    ...
    def test_the_broadcast_carries_the_redaction_flag(self) -> None:
        ...
        assert (
            '"message_redacted": safe_message != loop.message' in block
