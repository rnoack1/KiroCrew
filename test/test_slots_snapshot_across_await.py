...
async def test_cancelling_the_folder_delete_after_the_commit_still_unfiles_the_slots(
    ...
    the loaded vocabulary on purpose (``prune_unknown=False``) -- so this window is now a
...
async def test_superseded_folder_adoption_validates_the_observed_value(
    ...
    * well-formed and KNOWN   -> still adopted (the control)
    ...
    assert await _run("f_ghost") == "f_ghost", (
        ...
        "This site passes prune_unknown=False and validates SHAPE only, which is why the "
...
def test_the_tag_prune_asymmetry_self_retires_when_it_is_closed() -> None:
    ...
    `folder_id_for_restore` carries ``prune_unknown``; `tag_ids_for_restore` does not, so a
    readable-but-stale ``tags.json`` still strips tags at boot. That is a deliberate
    deferral, and the moment someone adds the parameter this gate FAILS -- forcing the
    now-false declaration and its spec paragraph to be deleted in the same change rather
    than left behind contradicting the code.

    Keyed on the SIGNATURE only. An earlier revision also asserted two docstring
    substrings, which made rewording a comment red the build for no correctness reason.
    """
    ...
    assert "prune_unknown" not in signature, (
        "tag_ids_for_restore now takes prune_unknown, so the asymmetry is CLOSED. Delete "
...
def _routes_through_a_validator(node: ast.Assign) -> bool:
...
def find_unvalidated_vocabulary_writes(
    ...
    for scope in ast.walk(tree):
        ...
        for node in _scope_nodes(scope):
            ...
            if attr is None:
                continue
            if _routes_through_a_validator(node):
                continue
...
async def test_a_failed_vocabulary_write_does_not_unfile_the_folders_conversations(
    ...
    assert (
        state.folder_id_for_restore("f1", prune_unknown=False) == "f1"
...
def test_an_observed_unknown_vocabulary_is_not_treated_as_no_observation(tmp_path) -> None:
    ...
    assert state.folder_id_for_restore("f1") == "", (
        ...
        "the cold-start `prune_unknown=False` sites rely on being separate from"
    )
