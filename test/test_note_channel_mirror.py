...
class TestTheProactiveSlackTierMapIsMechanical:
    ...
    Proactive Slack egress comes in three tiers: the hardened chain in
    ``slack_egress``, the gate-only compaction notice, and the plain-client senders
    that still carry a live TOCTOU. Two reviewers made the same point about that
    map -- it is written in comments, and a comment cannot fail, so nothing stopped
    a fourth plain-client sender arriving or the consolidation being forgotten.

    This census is that check. Scoped to ``dashboard`` deliberately: the ``slack``
    package answers inbound events, where replying to a message the user just sent
    has no proactive TOCTOU to close. A new direct sender fails this test, so its
    author has to say which tier it joins rather than leave it unclassified. Pinned
    by MODULE, not by line, so ordinary edits inside these files do not move it.
    """
