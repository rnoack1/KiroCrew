...
class TestNegativeControls:
    ...
    def test_neither_trailer_swallows_the_other_marker(self):
        ...
        for text in (options_first, action_first):
            ...
            if content:
                assert ACTION_HEAD not in content.group("labels"), text
...
class TestSameLineMixedMarkers:
    ...
    A line tail anchored at end-of-line matches only the TRAILING marker on a shared
    line, leaving the leading one unmatched — which on the backend means its raw text
    is posted verbatim into a Slack body, spoken by TTS, and left
    ...
    def test_the_content_body_is_captured_not_swallowed(self):
        ...
        assert match is not None
        assert match.group("labels").strip() == "Alpha | Beta"
...
class TestMixedCaseActionSiblingDoesNotCorruptTheContentMarker:
    ...
    def test_the_content_body_is_not_the_corrupted_capture(self):
        match = OPTIONS_RE_LINE.search(self.MIXED)
        assert match is not None
        assert match.group("labels") != self.CORRUPTED
        assert match.group("labels").strip() == "A"
    ...
    def test_no_casing_of_the_action_head_leaks_into_a_content_label(self):
        ...
        for head in (
            ...
            assert match.group("labels").strip() == "A", (head, match.group("labels"))
    ...
    def test_negative_control_the_content_head_is_NOT_widened(self):
        ...
        explicitly out of scope. ``Options:`` is not a recognised head, so the
        mixed-case CONTENT sibling is ordinary text on the same line, and under the
        conditional-closer rule a marker trailed by ordinary text DECLINES rather than
        running its body to the last closer — the same protection that keeps
        ``Use [OPTIONS: A | B] then check arr[0]`` visible.

        Still the control it was: under a blanket ``IGNORECASE`` the sibling WOULD
        be a recognised head, the tail's sibling-lookahead would fire, and this
        would match with ``labels == " A"``. So it fails if anyone reaches for the
        blanket flag, which is the only way the pair above can be trusted.
        """
        assert OPTIONS_RE_LINE.search("[OPTIONS: A] [Options: B]") is None
...
class TestOnlyOneBackendFileDefinesTheMarkerPatterns:
    ...
    def test_the_allowlist_carries_no_stale_entry(self):
        """An exemption that does not apply is a hole left open by accident.
...
class TestPreExistingOptionsBehaviourIsUnchanged:
    ...
    def test_the_three_intended_changes_are_the_ONLY_ones(self):
        ...
        # 2. A bracket INSIDE a label leaves no tail to be spoken: a lazy scan stops
        #    at the first closer and lets `| B]` survive into the utterance.
...
def test_no_hand_rolled_options_literal_remains_in_voice_reply():
    ...
    source = re.sub(r"#.*", "", (vr.__file__ and open(vr.__file__, encoding="utf-8").read()) or "")
...
class TestSomethingActuallyEmitsTheMarker:
    ...
    def test_positive_control_the_probe_can_see_taught_marker_syntax(self):
        ...
        did not reach this constant, the assertion above would fail — but so
...
class TestMarkerStripBodyIsLinearTime:
    ...
    #: A body shape the module must not contain. Reconstructed rather than imported,
    #: because the point is that it is absent from the module.
    _SUPERSEDED_BODY = rf"(?:[^[\]\n]|{_TEMPER}[^[\]\n]*{_MARKER_CLOSE_CLASS}?)*"
...
class _SlicedCharCounter(str):
    """A ``str`` tallying in :attr:`copied` every character sliced out of it.

    The two excision shapes in :class:`TestSettleMarkerHoldIsLinear` are compared by
    this tally rather than by elapsed time, because characters-copied IS the quantity
    the quadratic shape spends: it re-derives the whole residue per span, where the
    linear pass copies each character exactly once. A wall-clock ratio between them
    measures a shared runner's load as much as the complexity claim, so it cannot be
    made reliable at any threshold; the tally is an integer and reproduces exactly.

    Slicing and concatenation both return this type again, and that PROPAGATION is
    what makes the instrument able to fail. A quadratic shape rebinds its residue to
    ``residue[:head] + residue[closer + 1:]``, so a counter that returned plain
    ``str`` would go blind after the first span and report a tally of exactly *n* --
    indistinguishable from the linear pass, measured at 1.0x against a deliberately
    quadratic implementation. Propagating, the same comparison reads 1200x.
    """
    ...
    def __getitem__(self, key):
        ...
        _SlicedCharCounter.copied += len(out)
        return _SlicedCharCounter(out)

    def __add__(self, other):
        return _SlicedCharCounter(str.__add__(self, other))

    def __radd__(self, other):
        return _SlicedCharCounter(str.__add__(str(other), self))


def _chars_sliced_by(shape, residue: str) -> int:
    """How many characters *shape* slices out of *residue*, as an exact integer."""
    _SlicedCharCounter.copied = 0
    shape(_SlicedCharCounter(residue))
    return _SlicedCharCounter.copied
...
class TestSettleMarkerHoldIsLinear:
    ...
    #: An excision shape the module must not contain: one slice-and-concat per span,
    #: each O(n). Reconstructed rather than imported because it is absent from the module.
    @staticmethod
    ...
    def test_the_held_line_actually_reaches_the_excision(self):
        """POSITIVE CONTROL for the bound and the copy-count comparison below.
        ...
        survives. Pinned here because if that ever changed, both would keep passing
        while measuring an empty string.
        """
        held = self._held_line(50)
        residue = _strip_line_markers(held)

        assert len(residue) == len(held), (
            "the line strip consumed this fixture, so the assertions below "
    ...
    def test_the_superseded_excision_blows_up_on_the_same_input(self):
        """NEGATIVE CONTROL for the bound above, by characters copied not by clock.

        Without this, that bound would pass just as happily against an implementation
        that never had the defect, so it would prove nothing about having fixed one.

        What separates the shapes is how many characters each SLICES out of its input,
        which is the complexity claim itself rather than a proxy for it: doubling the
        span count must roughly DOUBLE that figure for the current pass and roughly
        QUADRUPLE it for the shape the module must not contain. Both figures are exact
        integers, so no runner load, GC pause or descheduling can enter the comparison.
        """
        small = _strip_line_markers(self._held_line(1000))
        large = _strip_line_markers(self._held_line(2000))

        superseded_small = _chars_sliced_by(self._superseded, small)
        superseded_large = _chars_sliced_by(self._superseded, large)
        current_small = _chars_sliced_by(excise_marker_spans, small)
        current_large = _chars_sliced_by(excise_marker_spans, large)

        assert superseded_small > len(small) * 10, (
            "the superseded excision no longer re-derives the residue per span, so it "
            "is not a control -- either the reconstruction above stopped matching what "
            f"was replaced, or the counter stopped propagating (copied {superseded_small} "
            f"for a {len(small)}-char input)"
        )
        assert superseded_large / superseded_small > 3.5, (
            f"the superseded shape grew {superseded_large / superseded_small:.2f}x on a "
            "2x input, so it is measuring as linear and cannot be the quadratic control"
        )
        assert current_large / current_small < 2.5, (
            f"the current pass grew {current_large / current_small:.2f}x on a 2x input "
            "-- linear predicts 2.0 and quadratic 4.0, so the quadratic excision is back"
        )
        assert current_large * 50 < superseded_large, (
            f"only {superseded_large / current_large:.0f}x apart (superseded "
            f"{superseded_large} chars, current {current_large}) -- the fixture no "
            "longer separates the two shapes"
        )
...
class TestTrailingSuffixSplitIsLinear:
    ...
    def test_growth_is_linear_not_quadratic(self, monkeypatch):
        """Doubling the marker count must not more than double the anchored matches.

        Counts the work instead of timing it. The claim is algorithmic — the scan finds
        every head once, then does a fixed number of anchored matches per head — so the
        honest measurement is how many matches happen, which is exact on any machine. A
        CPU-time ratio cannot carry the same claim: linear predicts 2.0 and quadratic 4.0,
        and a shared runner's noise spans that gap, so the timing form failed CI at 3.33
        while this implementation measured a flat 2.374us per marker from k=1000 to
        k=32000. A reintroduced prefix rescan still fails here, and harder: it would make
        the count grow with k^2 rather than drift a ratio upward.
        """
        calls = {"n": 0}
        inner = _MarkerMatcher.match

        def counting(self, *args, **kwargs):
            calls["n"] += 1
            return inner(self, *args, **kwargs)

        # Patched on the CLASS: the matchers use __slots__, so a per-instance override is
        # read-only, and the class hook counts every matcher the scan consults anyway.
        monkeypatch.setattr(_MarkerMatcher, "match", counting)

        def matches(k: int) -> int:
            text = "body. " + (self.MARK + "\n") * k
            calls["n"] = 0
            split_trailing_protocol_suffix(text)
            return calls["n"]

        small, large = matches(2000), matches(4000)
        # The counter must be wired: a zero would make the ratio below vacuously fine.
        assert small > 0, "no anchored match observed -- the counter is not wired"
        assert large == 2 * small, (
            f"{small} matches at k=2000 but {large} at k=4000 -- expected exactly "
            f"{2 * small}; a super-doubling count is the quadratic rescan returning"
        )


class TestTrailingSuffixSplitPreservesTheRunSemantics:
    """The linear walk must peel exactly the markers an anchored search would peel.
