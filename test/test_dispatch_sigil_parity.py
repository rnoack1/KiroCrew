...
class TestDispatchProtocolListsAgree:
    ...
    def test_neither_surface_carries_a_protocol_the_other_lacks(self):
        """Both guards spell the SAME literal, so neither surface can drift from the other."""
        backend = (_ROOT / "src" / "kiro_crew" / "constants.py").read_text(encoding="utf-8")
        assert 'probe.startswith("action::")' in backend
        assert "dispatched.startsWith('action::')" in _FRONTEND.read_text(encoding="utf-8")
