...
class TestPersistedStatusFlagIsNormalised:
    ...
    Without it a reader has to test ``is True``, because a legacy or
    hand-edited ``tags.json`` can carry the STRING ``"false"`` and a string is
    truthy. That puts the burden on five separate readers to remember the same
    trick, and they disagree: the drop handler classifies an inactive tag as a
    board lane while the lane filter does not, so the very drop that should
    fire ``SessionLaneChanged`` does not. Normalising once at the boundary is
