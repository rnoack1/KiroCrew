...
#: Smallest ``small`` reading a ratio may be computed from: several Windows clock grains.
COARSE_CLOCK_FLOOR_S = 0.05
...
class TestMarkerFilterStaysLinear:
    ...
    def test_scales_linearly_on_a_bracket_heavy_candidate_line(self):
        ...
        Sized so both terms clear the process clock's grain. Windows schedules that
        clock in ~15.6ms steps, so a sub-grain reading quantizes -- one step against
        four reads as exactly 4.00x, indicting the scan for the clock's rounding.
        """

        def run(closers: int) -> float:
            text = "[OPTIONS: " + ("x]" * closers) + "\n"
            best = min(_time_once(text) for _ in range(7))
            return best

        small = run(32_000)
        large = run(64_000)
        # A COARSE-CLOCK floor, not a relaxed budget: the multiplier below is unchanged.
        if small < COARSE_CLOCK_FLOOR_S:
            pytest.skip(
                "process clock too coarse: %.4fs is under the %.3fs floor"
                % (small, COARSE_CLOCK_FLOOR_S)
            )
