...
def test_restore_parks_over_ceiling_entries_instead_of_discarding_them():
    """GPT BLOCKING (round three): handover overflow discarded acknowledged context.

    A handover can leave more acknowledged entries on one metadata line than a single
    queue seats, because `append_pending_context` enforces `_MAX_PENDING_CONTEXT` and
    its own docstring makes "the queue only ever holds what can be persisted" an
    invariant. Dropping the surplus on the way back in is what DELETES it: the next
    save writes the seated queue and the rest is gone.

    Parking keeps them verbatim -- `export_pending_context` writes `_ctx_held_foreign`
    back -- so a later restore seats them once a drain has freed room.
    """
    ...
    parked = [e["ctxId"] for e in (getattr(slot, "_ctx_held_foreign", None) or [])]
    assert len(seated) == _MAX_PENDING_CONTEXT, f"the live ceiling still holds: {len(seated)}"
    # The property that matters: NOTHING acknowledged was dropped on the floor.
    assert sorted(seated + parked) == sorted(e["ctxId"] for e in entries), (
        f"every acknowledged entry must survive: seated {len(seated)}, parked {len(parked)}, "
        f"of {len(entries)}"
    )
    # And they are re-exported, so the next save cannot erase them.
    exported = {e["ctxId"] for e in slot.export_pending_context()}
    assert exported == {e["ctxId"] for e in entries}, "the surplus must be persisted too"
...
def test_every_context_filter_preserves_its_originating_transcript():
    ...
    guarded = src.count("_writing_origin and e in _held_ctx")
...
def test_no_pre_replay_row_is_seated_at_a_refused_binding():
    ...
    # The reporting GPT told us to retain must still be there. Two call sites in
    # THIS file (the rehydrate and the resume replay); the other two adoption sites
    # live in chat_handlers and channel_slots.
    assert "so the slot stays unbound and answers from its own" in src
    assert src.count("audit_persisted_binding(") == 2, (
        f"expected 2 SEL audit call sites in chat_persistence, found "
        f"{src.count('audit_persisted_binding(')} -- the refusal reporting moved"
    )
...
def test_queue_comments_describe_the_behaviour_the_code_actually_has():
    ...
    assert restore.count("PARKED, NOT DISCARDED") == 2
    # Positive control: the park itself, for the same reason.
    assert restore.count("self._ctx_held_foreign = [") == 2
