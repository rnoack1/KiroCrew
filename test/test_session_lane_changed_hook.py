...
class TestAppTokensCannotTriggerLaneHooks:
    ...
    @pytest.mark.asyncio
    async def test_the_gate_records_its_authorization_decision(self):
        """The gate records the decision it makes, refusal or permission alike.

        Leaving the permitted case to the per-run row left the default state unrecorded:
        with no hook subscribed nothing runs, so the trail held denials and silence, and
        silence meant both "refused" and "allowed, nothing listening". One row per
        authorized dispatch, off the request path, is what tells those apart.
        """
        from kiro_crew.dashboard import chat_tags as CT
        ...
        assert audits == [], (
            "the allow arm filed %d row(s). The gate is only reached once a lane hook is "
            "registered, so the permitted decision is already recorded per run and a row "
            "here is a third for one decision" % (len(audits),)
        )
...
class TestALaneIsWhateverTheBoardCallsOne:
    """Truthiness, and served as stored -- the predicate must not reclassify a lane.

    A stricter predicate turns a hand-edited `"status": "false"` from a lane into an ordinary
    tag, and narrowing the served value does the same thing to the browser's own truthy
    readers. Both are changes to existing board behaviour rather than additions.
    """

    def test_a_spelled_off_status_is_still_a_lane(self):
        from kiro_crew.dashboard.chat_tags import _is_status_tag

        for spelling in ("false", "no", "off", "0", "maybe"):
            assert (
                _is_status_tag({"id": "t", "status": spelling}) is True
            ), "a persisted lane was reclassified as an ordinary tag: %r" % (spelling,)

    def test_a_genuinely_empty_status_is_not_a_lane(self):
        from kiro_crew.dashboard.chat_tags import _is_status_tag

        # The control: without this the predicate could admit everything and the drop path
        # would strip tags that were never lanes.
        for tag in ({"id": "t"}, {"id": "t", "status": ""}, {"id": "t", "status": 0}, None):
            assert _is_status_tag(tag) is False, "a non-lane counted as one: %r" % (tag,)

    @pytest.mark.asyncio
    async def test_the_get_path_serves_status_as_stored(self, tmp_path):
        state = _make_state(tmp_path)
        state._tags = [
            {"id": "spelled-off", "name": "SpelledOff", "status": "false", "order": 0},
            {"id": "real", "name": "Real", "status": True, "order": 1},
        ]
        app = _make_tags_app(state)

        async with TestClient(TestServer(app)) as client:
            served = await (await client.get("/api/chat/tags")).json()

        by_id = {t["id"]: t for t in served}
        assert by_id["spelled-off"]["status"] == "false", (
            "the stored value was narrowed on the way out to %r, so the browser's truthy "
            "readers stop treating a persisted lane as one" % (by_id["spelled-off"]["status"],)
        )
        # The control: a real lane must still arrive intact.
        assert by_id["real"]["status"] is True


class TestDisablingAQueuedHookStopsIt:
    """A disable that lands after the fire targets were frozen must still take effect.

    `_committed_fire_targets` deep-copies the committed hooks so a rolled-back update's
    command cannot execute. That freezes `enabled` with it, and firing then awaits a matcher
    offload -- exactly where an operator's disable arrives. This drives `fire()` and disables
    the hook from inside that await, so it fails if only the frozen copy is consulted.
    """

    @pytest.mark.asyncio
    async def test_a_hook_disabled_during_the_matcher_await_does_not_run(self, tmp_path):
        store = hooks.ScriptHookStore(config_dir=tmp_path)
        hook = hooks.ScriptHook(
            id="h1",
            name="lane",
        ...
        store._hooks = {"h1": hook}
        store._snapshot = [hook]

        ran: list[str] = []

        def _matcher_disables_it(*_a, **_k):
            # The operator's disable commits while firing sits on this offload.
            store._snapshot = [
                hooks.ScriptHook(
                    id="h1",
                    name="lane",
        ...
        async def _spy(h, *_a, **_k):
            ran.append(h.id)
            return hooks.ScriptHookResult(
                hook_id=h.id, hook_name=h.name, event=h.event, exit_code=0
            )

        with (
            patch.object(hooks, "_context_matches", _matcher_disables_it),
            patch.object(hooks, "run_script_hook", _spy),
        ):
            await store.fire("SessionLaneChanged", context="added:done;")

        assert ran == [], "the hook ran from its frozen copy after being disabled mid-loop: %r" % (
            ran,
        )

    @pytest.mark.asyncio
    async def test_a_hook_left_enabled_still_runs(self, tmp_path):
        # The control: without this the guard could refuse everything and nothing would fire.
        store = hooks.ScriptHookStore(config_dir=tmp_path)
        hook = hooks.ScriptHook(
            id="h1",
            name="lane",
        ...
        with (
            patch.object(hooks, "_context_matches", lambda *_a, **_k: True),
            patch.object(hooks, "run_script_hook", _spy),
        ):
            await store.fire("SessionLaneChanged", context="added:done;")

        assert ran == ["h1"], "an enabled, matching hook did not run: %r" % (ran,)


class TestAHandNamedLaneReachesTheMatcher:
    """A dotted lane id reaches the matcher, and a refused one says so.

    The allowlist protects the token grammar -- whitespace splits a token, `:` forges the
    opposite direction, `;` terminates one, `*`/`?` are consumed by fnmatch. `.` does none of
    those. Case is deliberately still refused: matching folds it, so two ids differing only in
    case would share a token and fire each other's hooks.
    """

    def test_a_dotted_id_is_admitted(self):
        for tid in ("in.review", "done", "a1b2c3d4e5f6", "my_lane", "my-lane"):
            assert hooks._is_token_safe(tid) is True, "a usable id was dropped: %r" % (tid,)

    def test_the_grammar_breakers_are_still_refused(self):
        # The control: the widening must not have admitted the class the allowlist exists for,
        # nor the case-folding alias the companion test above pins.
        for tid in ("my lane", "a:b", "a;b", "*", "a*", "?", "", "Done", "MixedCase12"):
            assert (
                hooks._is_token_safe(tid) is False
            ), "an id that breaks the token grammar or aliases by case was admitted: %r" % (tid,)

    def test_a_refused_id_is_logged_rather_than_dropped_in_silence(self, caplog):
        import logging

        with caplog.at_level(logging.WARNING, logger="kiro_crew.hooks"):
            context = hooks._session_lane_matcher_context(["Done"], [])

        assert "Done" not in context, "an aliasing id reached the matcher context"
        assert any(
            "cannot be a matcher token" in r.getMessage() for r in caplog.records
        ), "the lane went missing from the context with nothing logged: %r" % (
            [r.getMessage() for r in caplog.records],
        )

    def test_an_admitted_id_is_not_logged(self, caplog):
        import logging

        # The control: without this the warning could fire on every id and mean nothing.
        with caplog.at_level(logging.WARNING, logger="kiro_crew.hooks"):
            context = hooks._session_lane_matcher_context(["in.review"], [])

        assert "added:in.review;" in context
        assert not [r for r in caplog.records if "cannot be a matcher token" in r.getMessage()]


class TestTwoWritersOnOneSlotEnqueueInCommitOrder:
    """The queue must receive lane transitions in the order they committed.

    Each dispatch awaits an off-loop audit thread before its enqueue. Dispatching outside the
    write lock lets two writers on one slot resume in thread-completion order rather than
    commit order, so the FIFO can hold the later transition first -- and a close-out hook that
    has already taken an external action cannot be undone by the re-read contract. Dispatching
    inside the lock makes commit order and queue order the same order by construction.

    The inversion is FORCED, not raced: the first caller through the gate is given the slower
    audit hop, so an enqueue that happens after that hop is guaranteed to arrive second.
    """

    @pytest.mark.asyncio
    async def test_the_later_commit_is_enqueued_last(self, tmp_path):
        state = _make_state(tmp_path)
        app = _make_tags_app(state)

        enqueued: list[tuple[str, ...]] = []

        async def _capture(_store, *, items, **_k):
            for item in items:
                enqueued.append(tuple(item.added))
            return True

        calls = {"n": 0}

        async def _allow_dispatch(_request, **_k):
            # The gate has its own tests; what this one measures is enqueue ORDER. Its own
            # audit hop is kept, because that hop is the thing that reorders the resumes.
            calls["n"] += 1
            await asyncio.sleep(0.05 if calls["n"] == 1 else 0.0)
            return True

        async def _skewed_audit(fn, _label):
            with contextlib.suppress(Exception):
                fn()
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            with (
                patch("kiro_crew.dashboard.chat_tags.save_slot_off_loop"),
                patch("kiro_crew.dashboard.chat_tags.audit_off_loop", _skewed_audit),
                patch(
                    "kiro_crew.dashboard.chat_tags._lane_dispatch_allowed",
                    _allow_dispatch,
                ),
                patch(
                    "kiro_crew.dashboard.chat_tags.dispatch_session_lane_changed_bulk",
                    _capture,
                ),
            ):
                # Both writers target the SAME slot, so the write lock decides commit order.
                responses = await asyncio.gather(
                    client.put("/api/chat/slots/s1/tags", json={"tags": [one]}),
                    client.put("/api/chat/slots/s1/tags", json={"tags": [two]}),
                )
                await _settle()

        assert [r.status for r in responses] == [200, 200], "a write was refused: %r" % (
            [r.status for r in responses],
        )
        assert len(enqueued) == 2, "both transitions must reach the queue: %r" % (enqueued,)
        final = tuple(state._slots["s1"].tags)
        assert enqueued[-1] == final, (
            "the queue holds the earlier transition last: enqueued %r while the slot committed "
            "%r last, so a subscriber reads the lane change inverted" % (enqueued, final)
        )


class TestTheSpecDescribesTheShippedPredicate:
    """The spec paragraph and the code must not be allowed to drift apart again.

    Both have been rewritten more than once on this change, and each time the loser was the
    paragraph -- which then described a mechanism the diff did not contain. Prose cannot be
    pinned by a unit test of behaviour, so this reads BOTH artefacts and compares them.
    """

    SPEC = pathlib.Path(__file__).resolve().parents[1] / (
        "docs/system-specs/modules/memory-skills-hooks.md"
    )
    SOURCE = pathlib.Path(__file__).resolve().parents[1] / ("src/kiro_crew/dashboard/chat_tags.py")

    def _spec_text(self) -> str:
        body = self.SPEC.read_text(encoding="utf-8")
        marker = "**What counts as a status tag, stated once.**"
        assert marker in body, "the status-tag paragraph is gone; this guard is blind"
        start = body.index(marker)
        return body[start : start + 1400]

    def test_the_spec_quotes_the_predicate_the_code_ships(self):
        shipped = 'return bool(isinstance(tag, dict) and tag.get("status"))'
        source = self.SOURCE.read_text(encoding="utf-8")
        assert shipped in source, "the predicate moved; update this guard with it"

        spec = self._spec_text()
        quoted = 'bool(isinstance(tag, dict) and tag.get("status"))'
        assert quoted in spec, (
            "the spec paragraph does not quote the shipped predicate, so it describes a "
            "mechanism the diff does not contain"
        )

    def test_the_spec_does_not_still_claim_the_coercer(self):
        # The control: the assertion above passes on a paragraph that quotes the predicate AND
        # goes on to describe the coercion it replaced, which is the exact contradiction.
        spec = self._spec_text()
        for stale in ("_coerce_bool", "accept_ints", '`"maybe"`'):
            assert stale not in spec, (
                "the status-tag paragraph still describes the coerced predicate (%r), which "
                "the code no longer uses" % (stale,)
            )

    def test_the_spec_quotes_the_shipped_token_charset(self):
        import re

        body = self.SPEC.read_text(encoding="utf-8")
        hooks_src = (
            pathlib.Path(__file__).resolve().parents[1] / "src/kiro_crew/hooks.py"
        ).read_text(encoding="utf-8")

        declared = re.search(r'_TOKEN_ALLOWED = re\.compile\(r"(\\A\[[^\]]+\]\+\\Z)"\)', hooks_src)
        assert declared, "the allowlist could not be read from the source; guard is blind"

        assert (
            declared.group(1) in body or "[a-z0-9_.-]" in body
        ), "the spec does not state the shipped charset %r" % (declared.group(1),)
        assert (
            "(`[a-z0-9_-]+`)" not in body
        ), "the spec still states the pre-dot charset, which the validator no longer uses"
