...
def _script_hooks_capability_denied(session_key: str = "") -> str | None:
    ...
    THE OFF-LOOP BOUNDARY IS HERE, AT THE SEAM, and it is narrow on purpose.
    Resolving this gate is synchronous: ``governance_profiles._dir_fingerprint``
    walks ``profiles/`` and a reload reads each profile JSON. Callers in this
    module await it through ``asyncio.to_thread`` so that walk never runs on the
    gateway event loop.

    What that does NOT mean: the offload is scoped to this module's two call sites
    and fixes nothing for anyone else. Every caller outside this module still
    resolves inline, including ``dashboard/chat_runner.py`` on the async turn path,
    so the stall remains there. That is a DEFERRED follow-up, not a claim the
    hazard is gone -- and the whole of it (the caller census and how to re-derive
    it, the cause-level remedy in ``governance_profiles``, and the thread-safety
    consequence this offload creates) is recorded in ONE place, the owning module's
    own spec: ``docs/system-specs/modules/governance.md`` → "Tracked: resolution is
    synchronous, and two callers work around it". Deliberately not repeated here,
    because a fact restated in five places drifts in five, and this one already
    drifted twice.
    """
    from kiro_crew.platform.context import PlatformCompositionError
...
async def run_script_hook(
    ...
    #
    # Offloaded to a thread: resolving the gate walks ``profiles/`` synchronously
    # (``governance_profiles._dir_fingerprint`` → ``Path.iterdir``, plus reading
    # each profile JSON on a reload), so calling it inline stalls the gateway
    # event loop on slow storage. Same reason -- and the same remedy -- as the
    # ``_context_matches`` offload in ``fire``. A ``PlatformCompositionError``
    # still propagates through ``to_thread``, so the fail-closed CPP is intact.
    #
    # Offloaded because resolving the gate walks ``profiles/`` synchronously.
    # This is a WORKAROUND at one caller, not the fix, and the scope, the
    # cause-level remedy and the thread-safety consequence are tracked in ONE
    # place -- ``docs/system-specs/modules/governance.md`` → "Tracked: resolution
    # is synchronous, and two callers work around it". Deliberately not restated
    # here: it was, in four places, and the count inside it drifted twice while
    # this PR was open.
    sk = ""
    if hook_event:
        sk = str(hook_event.get("parent_session_key") or hook_event.get("session_key") or "")
    # Scoped to THIS event: main resolves the gate inline, so offloading unconditionally
    # changed execution context for four pre-existing events as a rider.
    if hook.event == HOOK_EVENT_SESSION_LANE_CHANGED:
    ...
    # Audited per execution, not once per pair: a repeat run would otherwise execute
    # code with no record of the permission that allowed it.
    #
    # SCOPED to this event. main audits only the skills-only arm, so emitting on every
    # command-hook run changed audit volume for four pre-existing events as a rider.
    if hook.event == HOOK_EVENT_SESSION_LANE_CHANGED:
        _audit_governance_hook_decision(
            sk,
...
class ScriptHookStore:
    ...
    async def fire(
        ...
        if event == HOOK_EVENT_USER_PROMPT_SUBMIT and context:
            ...
            #
            # Defaults are stamped for all four keys before the caller's mapping is
            ...
            hook_event.update({"slot": "", "added": [], "removed": [], "tags": []})
        ...
        for hook in list(self._hooks.values()):
            ...
            if (
                ...
                # Offloaded for the same reason as the ``run_script_hook`` site:
                # resolving the gate walks ``profiles/`` synchronously. BOTH inline
                # call sites are offloaded, so no arm of this gate runs the
                # directory walk on the event loop.
                #
                # Scope, cause-level remedy and the thread-safety consequence
                # this offload creates are tracked in ONE place --
                # ``docs/system-specs/modules/governance.md`` → "Tracked:
                # resolution is synchronous, and two callers work around it" --
                # so the follow-up outlives this feature.
                if event == HOOK_EVENT_SESSION_LANE_CHANGED:
                    gov_denied = await asyncio.to_thread(_script_hooks_capability_denied, sk)
                else:
                    gov_denied = _script_hooks_capability_denied(sk)
                if gov_denied:
...
# SessionLaneChanged delivery: ``_LANE_SHARDS`` bounded FIFO delta queues, one worker
# each, a session pinned to one shard. Order is per-session, never total.
#
...
#     one task to survive.
#   * ``_LANE_SHARDS`` bounds REAL RESOURCES -- a dispatch can run a hook command,
#     i.e. a subprocess with its own file descriptors, so the pool stays small.
#
# Each session hashes to ONE shard: exactly the per-session order the event promises,
# without one wedged hook deferring every other session -- see the module spec.
_LANE_QUEUE_MAXSIZE = 512
# Bounds the shard pool. Per-session order is all the event promises, so a wedged
# hook should stall its own shard rather than every other session's fires.
_LANE_SHARDS = 4
...
#
# Strong refs to the workers are held here because asyncio keeps only a weak
# reference to a task: without this a worker can be garbage collected mid-drain
# and the hooks it would have run silently never run.
_LANE_QUEUES: list[asyncio.Queue] | None = None
_LANE_WORKERS: list[asyncio.Task] | None = None
# The loop the queues and workers belong to. An ``asyncio.Queue`` and the tasks
# draining it are bound to ONE loop, and a test suite (or an in-process gateway
# restart) runs many; reusing them across loops would park deltas behind workers
...
def _reset_lane_dispatch_state() -> None:
    ...
    """
    global _LANE_QUEUE_LOOP, _LANE_QUEUES, _LANE_WORKERS
    for task in _LANE_WORKERS or []:
...
def _lane_shard(slot_key: str) -> int:
    """Which shard a session's deltas ride. STABLE, so one session keeps its order.

    Digested rather than ``hash()``: the builtin is salted per process, so an
    ordering guarantee resting on it could not be reproduced in a test.
    """
    digest = _hashlib.sha256(slot_key.encode("utf-8", "replace")).hexdigest()
    return int(digest[:8], 16) % max(1, _LANE_SHARDS)


def _lane_dispatch_queue(slot_key: str) -> asyncio.Queue | None:
    """This session's shard queue, creating the pool and its workers on first use.
    ...
    A session always resolves to the SAME shard, which is what preserves the only
    ordering the event promises. Order BETWEEN sessions was never promised, and one
    global FIFO delivered it only by letting one wedged hook defer every other
    session behind it.
    """
    global _LANE_QUEUE_LOOP, _LANE_QUEUES, _LANE_WORKERS
    try:
        ...
        return None
    shards = max(1, _LANE_SHARDS)
    if _LANE_QUEUES is None or _LANE_QUEUE_LOOP is not loop or len(_LANE_QUEUES) != shards:
        _LANE_WORKERS = None
        _LANE_QUEUE_LOOP = loop
        _LANE_QUEUES = [asyncio.Queue(maxsize=_LANE_QUEUE_MAXSIZE) for _ in range(shards)]
    if _LANE_WORKERS is None:
        _LANE_WORKERS = [asyncio.create_task(_lane_dispatch_worker(q)) for q in _LANE_QUEUES]
    return _LANE_QUEUES[_lane_shard(slot_key)]
...
def _session_lane_matcher_context(
    ...
    A subscriber wanting the human-readable label reads ``added``/``removed``/
    ``tags`` from the payload and resolves the ids it finds there.
...
def dispatch_session_lane_changed_bulk(
    ...
    *,
    ...
    for item in items:
        queue = _lane_dispatch_queue(item.slot_key)
        if queue is None:
            return
        try:
