...
def _audit_gate_row(session_key: str, hook_label: str, gating: bool = True) -> bool:
    ...
    SEL's criterion is ATTENDEDNESS, not blast radius, so ``gating`` tracks it. A lane change
    dispatches with nobody watching and gates. The turn-lifecycle events fire inside an
    attended turn, so their row is still written -- critically, so a failure is visible -- but
    it does not deny the run, because refusing there would trade a guard hook's availability
    for symmetry rather than for a reported defect. Returns False only when a GATING row
    ...
    try:
        ...
        if not gating:
            # Attended run: the trail loses a row and the run proceeds.
            logger.warning("hook start row could not be written", exc_info=True)
            return True
        logger.warning("hook start row could not be written; refusing the run", exc_info=True)
        return False
...
async def run_script_hook(
    ...
    # AUDIT-OR-DENY for the UNATTENDED event: an unrecordable start means the command would run
    # untraced with nobody watching, so it does not run. The attended events record but do not gate.
    gating = hook.event == HOOK_EVENT_SESSION_LANE_CHANGED
    start_write = asyncio.ensure_future(
        asyncio.to_thread(
            lambda: _audit_gate_row(sk, f"run_script_hook:{hook.name or hook.id}", gating)
        )
    )
    try:
    ...
    # No "allowed" row here: the invocation-outcome row this run files already proves it
    # was permitted, and a governance denial writes its own row above.
...
class ScriptHookStore:
    ...
    async def run_one_and_fold(
        ...
        ran = copy.deepcopy(committed if committed is not None else hook)
