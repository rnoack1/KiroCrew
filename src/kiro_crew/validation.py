...
def max_section_label() -> int:
    """Hard cap on a section label, read off :data:`SECTION_MARKER_SCHEMA`.

    Lives here, beside the schema, because three consumers need it — the applier,
    the transfer bundle validator and the deferred-hold restore. Each re-checks the
    cap rather than trusting an upstream validator, and none may re-SPELL the
    number: a second literal is a second source of truth, and the first time either
    moved the belt would silently stop matching the schema with no failure at the
    moment of divergence. Resolved through the module global at CALL time, so a test
    that patches the schema is observed.
    """
    for spec in SECTION_MARKER_SCHEMA.fields:
        if spec.name == "label":
            return spec.max_len
    # Loud rather than a fallback literal: no ``label`` field means the schema and
    # its readers have diverged, which is the one thing this function prevents.
