...
_REDACTION_SINKS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        ...
        "reaches `build_message`, so the model prompt stays verbatim. The two assistant "
        "appends in this module need no call here -- `_redact_at_write_boundary` already "
        "scrubs every role other than `user`, and the `user` exemption is exactly the gap "
        "this entry closes.",
    ),
    (
        "Webex delivery",
