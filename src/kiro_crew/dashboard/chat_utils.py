...
def _is_separator_fold(candidate: str, stem: str) -> bool:
    ...
    So the rule is positive, not exclusionary: a folded position must have carried the one
    separator a live session key is spelled with. ``messaging.link`` states that grammar --
    a live key uses ``:`` and ``_`` is only the persisted stem's spelling -- so ``:`` is the
    single character a genuine candidate can hold where its own transcript name holds ``_``.
    Every other spelling fails to resolve rather than passing a shape check.
