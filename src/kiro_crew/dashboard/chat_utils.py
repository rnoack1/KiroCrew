...
async def preaudit_persisted_binding(meta: object, transcript_key: str) -> bool | None:
    ...
    if not landed:
        return False
    if adoptable and not persisted_binding_is_adoptable(candidate, transcript_key):
        # RE-DECIDED AFTER THE AWAIT: the coexistence probe answered about the directory as it
        # was, and a transcript created during the audit makes this alias route at the wrong file.
        # AUDITED AS DENIED: the row above records an ALLOW while this decision is a refusal, so
        # without a second row the trail attributes the refusal to a verdict nothing acted on.
