...
async def _deliver_slack_governed(
    ...
    *,
    ...
    so this docstring restates no part of it.
    ...
    *relink* re-reads the binding for the post-await coordinate checks -- the
    ...
    #: Which authority named the recipient for the chunk being sent. A cell because the
    #: two callbacks are siblings and the per-chunk driver passes neither the other.
    sending_basis: list[str] = [""]
    ...
    def _authority_still_holds(basis: str, *, configured_active: bool | None) -> bool:
        ...
        DM id that was already resolved. The configured-channel ACTIVATION read is
    ...
    async def _permitted_to_send(before: str) -> bool:
        ...
        if not basis:
            return False
        # Re-derived per chunk, so the cell always names the basis that authorized
        # THIS part rather than an earlier one.
        sending_basis[0] = basis
