...
async def deliver_to_channel(
    ...
    *tool_name* is the SEL caller identity, so an operator can tell which sender
    ...
    if not resolved.channel_id:
        ...
        return False
    # The off-loaded resolve is unbounded, so confirm the permit was authorized under
    # the ceiling still installed rather than one replaced while it ran.
    if not governance_ceiling_unchanged(permit_ceiling):
        _audit("denied", "governance_changed_during_resolve")
        logger.warning(
            "channel send: governance ceiling for %s moved while the permit was being "
            "resolved; refusing rather than sending on a straddled authorization",
            session_key,
        )
        return False
    ...
    async def _still_permitted(index: int) -> bool:
        # Part 1 has no re-resolve, so the ceiling sampled before the resolve is what
        # says its permit still holds. Both failure arms of the shared check refuse.
        if not index:
            if governance_ceiling_unchanged(permit_ceiling):
                return True
            _audit("denied", "governance_changed_before_part_1")
            logger.warning(
                "channel send: governance ceiling for %s moved between resolve and the "
                "first part; refusing on the superseded permit",
                session_key,
            )
            return False
        ...
        if not _binding_unchanged(final_link, final_is_origin, link, link_is_origin):
            ...
            return False
        # LAST, and synchronous: the re-resolve evaluated governance under the ceiling
        # installed when it started, so a tightening during it returns a stale permit.
        if not governance_ceiling_unchanged(recheck_ceiling):
            _audit("denied", f"governance_changed_mid_send_before_part_{index + 1}")
            logger.warning(
                "channel send: governance ceiling for %s moved while the mid-send "
                "re-resolve ran; aborted before part %d of %d (%d already delivered)",
                session_key,
                index + 1,
                len(parts),
                index,
