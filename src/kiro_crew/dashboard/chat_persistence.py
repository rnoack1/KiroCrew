...
def preserve_unaccounted_context(
    ...
    *,
    ...
    the export through ignorance rather than delivery, and the full save used to erase it. So
    ...
    try:
        ...
        if meta.get("linked_session_key"):
            ...
            if _adoptable:
                ...
                # copy. The refusal is marked ONLY by the log line above and the SEL event;
                # there is no slot-state record and no user-visible row for it.
    ...
    *,
    ...
    if meta.get("linked_session_key"):
        ...
        if _adoptable:
            ...
            # on slot state, where ordering is not a constraint.
...
def _save_slot_to_history(
    ...
    *,
    ...
    # no longer names an entry THIS transcript owns, so its retirement read as ignorance.
    ...
    try:
        ...
        with state.conversation_log._locked(history_key):
            ...
            if rows_only and existing_meta and not line_is_this_slots:
                ...
                _own_ctx_ids = _ctx_id_set(_mine_ctx)
