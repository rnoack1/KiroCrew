...
async def api_chat_folder_delete(request: web.Request) -> web.Response:
    ...
    # A slot popped BEFORE this capture is in NEITHER list; every restore path revalidates
    # folder ids now, so the drop happens there rather than in this sweep.
    for slot in [*closing, *state._slots.values()]:
        ...
        cleared.append((slot, live_key, slot._placement_seq))
