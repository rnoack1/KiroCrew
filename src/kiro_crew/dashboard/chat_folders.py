...
async def api_chat_folder_delete(request: web.Request) -> web.Response:
    ...
    # that same object back into ``_slots`` still naming a deleted folder, and the
    # periodic flush then full-saves it from memory, so the dangling
    ...
    # Retained WITH the capture-time transcript key: a rebind inside the commit await moves
    # routing, so a key read after it pins the new record and not the one holding the id.
    ...
    # a folder_id naming a deleted folder.
    #
    ...
    # A slot popped BEFORE this capture is in NEITHER list, and the close/cleanup restores
    # revalidate TAGS only, so its folder id stays until a later folder op settles it.
    ...
    def _adopt_observed_placement(slot: Any, observed: dict, submitted: dict[str, object]) -> None:
        # A plain value compare: the ABA window it cannot see is a declared residual.
        ...
        # ``isinstance`` guard.
        # The committed vocabulary decides: an id absent from a KNOWN set is dropped here,
        # and an UNKNOWN vocabulary keeps it.
        slot.folder_id = state.folder_id_for_restore(observed.get("folder_id"))
    ...
    async def _persist_cleared_and_publish() -> None:
        for slot, pinned_key in cleared:
            ...
            # withholds the write when this slot is not the live ``_slots`` entry.
            # Held for the whole pass-two write: the sweep persists through
            # ``update_metadata_if``, which raises no flush guard of its own.
...
async def api_chat_slot_folder(request: web.Request) -> web.Response:
    ...
    async with _slot_meta_txn_lock(state):
        ...
        # Captured BEFORE the await below. The two reverts drop it when the committed
        # vocabulary is KNOWN and lacks it, and keep it while that vocabulary is UNKNOWN.
        ...
        if not await _unhide_folder(state, folder_id):
            # Validated, not verbatim: this await can resume after a delete of `previous`
            # returned, and reattaching it writes a placement the user never chose.
            slot.folder_id = state.folder_id_for_restore(previous)
        ...
        if not await save_slot_off_loop(
            ...
            if slot.folder_id == folder_id:
                # Same reader as the refusal above, for the same reason.
                slot.folder_id = state.folder_id_for_restore(previous)
                slot._folder_changed = previous_changed
