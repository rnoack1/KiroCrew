...
class DashboardPersistenceCoordinator:
    ...
    def flush_slot_now(self, owner: Any, slot: Any) -> None:
        ...
        # WHAT WOULD REACH DISK, not what the queue holds: an ephemeral-only slot exports nothing,
        # so saving it writes a message-less file for content asked to stay in memory.
        # circular import: chat_persistence imports dashboard_persistence's saver wiring at module
        # scope, so importing it here at module scope would close the cycle.
        from kiro_crew.dashboard.chat_persistence import durable_queued_context

        if not slot.messages and not durable_queued_context(slot):
            return
