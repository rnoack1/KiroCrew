...
async def _run_leg(
    ...
    *audit_timeout* and *audit_error* are supplied by the caller rather than chosen
    ...
    It orders the two legs OF ONE NOTE and nothing wider. Each note is dispatched in
    its own task, so two notes written close together still race that load against
    each other, and this ordering does not close that. Only fixing the store closes
    it, which is why the consolidation RFC owns the cold load rather than this
    module ordering more callers.
...
async def _deliver_slack(
    ...
    *slack_link* is the snapshot taken before dispatch. It is NOT re-resolved here:
    ...
    #
    # Tier membership is enumerated once, in the proactive-Slack-egress-consolidation
    # RFC, and pinned mechanically by this module's census test. Not restated here.
    #
...
async def _deliver_via_transport(
    ...
    if authored_link is None:
        ...
        if not getattr(state, "channel_transports", None):
            return
        # AUDITED, unlike the branch above: this install HAS a deliverable surface, so
        # refusing on a binding's arrival time is a permission decision, not an absence.
        from kiro_crew.dashboard.handlers.messaging import audit_channel_send
