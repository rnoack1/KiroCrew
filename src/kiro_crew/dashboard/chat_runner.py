    ...
    def _queue_recovery(
        ...
        *,
        ...
        _recovery_qid = slot.queue_insert(
            ...
            #
            # ``_recovery_meta`` is the base rather than ``containment_meta``
            # directly: it already carries the replay's author provenance
            # (``model_authored`` / ``action_context``), and dropping those here
            # would re-enable command dispatch on a model-authored label.
            meta={
                **_recovery_meta,
