...
async def _consume_pending_reset(
    ...
    if slot._pending_reset_history_key:
        ...
        # Armed BEFORE any branch and regardless of outcome, keyed on `current_key` so a
        # rebind arms where the reset will actually land -- and TRANSFERS, so a rebind
        # leaves no arm behind on the key the slot abandoned. The cleared case is resolved
        # off-thread first: its per-session default stats and realpaths the workspace root.
        armed_cwd = slot.project or await state.sessions.resolve_arm_cwd(current_key, CWD_CLEARED)
        state.sessions.transfer_retire_arm(pending_key, current_key, armed_cwd)
