...
def context_entry_expired(entry: dict, now: float) -> bool:
    ...
    doing the arithmetic, which would raise ``TypeError`` on
...
class _ChatSlot:
    ...
    # NOTE on the decomposition: it moved the queue mutation into
    ...
    def pending_context_budget_room(
        self, entry: dict[str, Any], replacing: dict[str, Any] | None = None
    ) -> bool:
        """True if *entry* fits the PERSISTENCE budget alongside the live queue.

        ``replacing`` names an ALREADY-SEATED entry that *entry* supersedes in place, and
        is excluded from the tally below. The caller is the keyed-repost promotion, which
        lifts a memory-only entry to durable: that entry holds its seat either way, and
        charging a second one refuses every promotion on a queue at the ceiling. Its BYTES
        do change, from zero (withheld from the export) to its full cost, so the byte arm
        must still charge -- which is exactly what excluding it and re-adding *entry*
        computes. Passing it through here rather than doing the arithmetic at the call site
        keeps one capacity chokepoint.
        ...
        drain that would free the seats runs only on a turn. Eviction hides
        ...
        if replacing is not None:
            # BY IDENTITY, not equality: two entries can carry identical content, and
            # dropping both would under-count the queue.
            live = [e for e in live if e is not replacing]
        ...
        used = sum(
            max(self._context_entry_cost(e), 0) for e in live if e.get("ephemeral") is not True
        )
        ...
        for note in self._deferred_notes:
            ...
            seats += 1
            # SYMMETRIC with `used` and the arrival: an ephemeral entry is withheld from the
            # export, so reserving its bytes refuses a durable post over disk it never occupies.
            if ctx.get("ephemeral") is True:
                continue
        ...
        charge = 0 if entry.get("ephemeral") is True else cost
    ...
    def prune_ctx_owners(self, keep_ids: set[str] | None = None) -> int:
        """Drop owner records for ``ctxId``s this slot holds nowhere.

        Both writers above only ever insert, so the map grows in the count of entries
        EVER committed rather than in the seat-capped live queue -- a long-lived slot
        accumulates one record per entry it has ever acknowledged.

        Held foreign entries are retained: an entry whose owner is unknown reads as
        unowned, and an unowned foreign entry is adoptable, which is exactly the
        cross-session copy this map exists to prevent. Returns the number dropped.
        """
        owners = getattr(self, "_ctx_owner_by_id", None)
        ...
        keep = live | {c for c in (keep_ids or set()) if isinstance(c, str) and c}
        dropped = [cid for cid in owners if cid not in keep]
        if dropped:
            self._ctx_owner_by_id = {c: o for c, o in owners.items() if c in keep}
        return len(dropped)
    ...
    def promote_overflow_context(self) -> int:
        ...
        for entry in overflow:
            if context_entry_expired(entry, now):
                continue
            # ORDER-PRESERVING SUFFIX: `remaining` short-circuits the seat attempt, so a smaller
            # entry cannot be promoted past one still waiting for a seat ahead of it.
            if remaining or not self.append_pending_context(entry):
    ...
    def export_pending_context(self) -> list[dict[str, Any]]:
        ...
        # not prove a persisted binding belongs to it does not delete the copy: they
        ...
        overflow = getattr(self, "_ctx_overflow", None) or []
        # A HELD NOTE'S CONTEXT HALF IS NOT EXPORTED HERE. The note is its durable home, and
        # its flush promotes the context into this queue; exporting it too seats two copies.
        ...
        _out: list[dict] = []
        for e in [*inflight, *self._pending_context, *overflow, *held]:
    ...
    def restore_pending_context(self, entries: object) -> None:
        ...
        * ``maxAge`` / ``injectedAt``, when present, must be finite numbers, and
          ...
          :func:`context_entry_expired` does not raise on a bad one, but a
        ...
        * ``source`` is interpolated straight into the
        ...
        for entry in entries:
            ...
            if _note_authorized_elsewhere(entry, auth_key):
                ...
                continue
            seated = dict(entry)
            # STAMPED HERE, never read from the file: this is the one chokepoint every
            # restored entry passes, so a forged flag can only make content less trusted.
            seated["restoredFromDisk"] = True
            if not isinstance(seated.get("ctxId"), str) or not seated["ctxId"]:
                # The save's accounting keys on `ctxId`, so an entry without one is never
                # retired and reinjects on every start. DERIVED, so a restore is idempotent.
                _ident = json.dumps(
                    [
                        seated.get("content"),
                        seated.get("source"),
                        seated.get("injectedAt"),
                        seated.get("maxAge"),
                    ],
                    sort_keys=True,
                    default=str,
                )
                seated["ctxId"] = "legacy-" + hashlib.sha256(_ident.encode()).hexdigest()[:24]
            if not _usable_context_source(seated.get("source")):
                seated.pop("source", None)
            _expired = context_entry_expired(seated, time.time())
            # ORDER-PRESERVING SUFFIX: no seat attempt once one entry parked, so a later smaller
            # entry cannot re-seat ahead of it. An EXPIRED entry is still dropped, not parked.
            _seated = False if (_park_rest or _expired) else self.append_pending_context(seated)
            if not _seated and not _expired:
                _park_rest = True
