...
class DashboardState:
    ...
    def folder_id_for_restore(
        ...
        *,
        ...
        One helper rather than a copy at each site that adopts a persisted or derived
        ``folder_id``. The three ORIGINAL copies -- the two restore paths and the
        metadata branch of ``surface_channel_session`` -- were byte-identical, which
        is exactly how the fail-open rule below drifts apart later: a reader who
        fixes one copy has no reason to look for the others.

        TWELVE call sites route through it now: those three, plus the default-filing
        branch of that same helper, the superseded-merge adoption in
        ``api_chat_folder_delete``, the History-resume path in ``chat_handlers``, the
        refused-move reverts in BOTH ``api_chat_slot_folder`` and
        ``api_chat_slot_create`` -- each restores a ``folder_id`` captured before an
        await, so a delete landing inside that window leaves it naming a folder that
        is gone -- the persist-failure rollback in ``api_chat_slot_folder``, which is a
        SECOND revert on that same path (the refused-move one answers
        ``_unhide_folder``'s verdict, this one answers a refused save and is
        compare-and-set guarded), the two close-failure reinsertion sites
        (``close_slot`` and ``api_chat_slots_cleanup``), which revalidate a popped slot
        on the way back in when its save fails, and the fork's inheritance copy in
        ``api_chat_slot_fork``. The two refused-move
        reverts are siblings and had to be fixed together: the create path has no
        pre-check at all, relying solely on ``_unhide_folder``'s lock-held verdict, so
        its revert is the only place its stale capture can be caught.

        The fork is the PRODUCER, and it is here for a different reason from the
        other eleven: those adopt a value from persistence, while the fork copies one
        live slot's value onto a NEW record. Being unvalidated made it the only
        writer the delete sweeps could not see, because the record it writes does not
        exist when any snapshot is taken. Validating it removes that producer rather
        than chasing it downstream.

        That last one is the reason to keep counting: it was left out originally
        ...
        1. MALFORMED (anything that is not a ``str``) is dropped unconditionally.
           ...
           contract. With the vocabulary prune withheld at cold start, dropping a
           malformed value is the ONLY effect those four callers have.
        ...
        if not isinstance(value, str):
            ...
            # arm was silent, and it is now the only drop a cold-start restore makes.
        ...
        if not prune_unknown:
            # Absence cannot tell "deleted" from "folders.json is readable but STALE", and a
            # caller that did not author this value cannot date it. See history.md.
            return value
        ...
        # across its twelve call sites. Reached only by the eight await-window callers now,
        # so a stale vocabulary no longer produces drops here. See history.md.
    ...
    def tag_ids_for_restore(self, values: list[str]) -> list[str]:
        ...
        * ``frozenset()`` is a KNOWN-EMPTY vocabulary -- the user deleted the last tag.
        ...
        NO ``prune_unknown`` COUNTERPART, deliberately, and the folder side is now the
        SAME shape: a readable-but-stale ``tags.json`` parses as KNOWN, so the rule above
