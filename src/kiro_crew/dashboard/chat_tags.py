...
def _lane_dispatch_is_permitted(request: web.Request, *, resources: str = "") -> bool:
    ...
    if claim == "":
        # Audits the permission DECISION itself, so an ALLOW is recorded even though
        # a hook that runs is audited again at dispatch; both branches leave a record.
        sel().log_api_access(
            caller="dashboard",
...
async def api_chat_tag_delete(request: web.Request) -> web.Response:
    ...
    async with _tags_write_lock(state):
        ...
        if stripped_holders and _lane_dispatch_is_permitted(request, resources=tid):
            dispatch_session_lane_changed_bulk(
                get_global_hook_store(),
