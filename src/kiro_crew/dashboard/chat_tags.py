...
def validate_folder_tag_ids(raw: Any, state: DashboardState) -> list[str]:
    ...
    * AUTHORITY-GATED vocabulary intersection — DELEGATED to
      ...
      load-bearing. A dangling id kept that way survives restart, and is dropped by
      the first mid-session revalidation that can observe the delete.
...
async def _commit_tags_snapshot(state: DashboardState, snapshot: list[dict]) -> None:
    ...
    No ``rollback`` is passed: EACH MUTATING CALLER OWNS ITS OWN ROLLBACK, and for one of
    them that owner is a level further out. ``_mutate_tags_locked``,
    ``create_tag_definition_off_loop``, ``api_chat_tag_update`` and ``api_chat_tag_delete``
    restore ``state._tags`` on their own failure arm. ``persist_tags_snapshot_unlocked``
    carries no failure arm at all -- it is a lock-held pass-through that snapshots and
    delegates -- so its rollback belongs to ITS caller, ``maybe_auto_tag`` in
    ``chat_auto_tag``, which drops the appended definition and returns when this raises. The
    shared helper propagates the write's own failure rather than the cancellation precisely
    so those arms are reached. Centralising rollback here would need one undo shape for
    several different pre-images, which is why it stays with whoever holds the pre-image.
    """
...
async def api_chat_tag_delete(request: web.Request) -> web.Response:
    ...
    async with _tags_write_lock(state):
        ...
        # Boot does NOT keep it: the cold-start readers prune against the vocabulary they
        # just loaded, so the durable window closes at the next restart.
        #
        ...
        # Retained WITH the capture-time transcript key, as on the folder side: the object
        # alone lets a rebind inside the commit await move the pin onto the new transcript.
        ...
        for slot, key_at_capture in [*closing, *((s, None) for s in state._slots.values())]:
            ...
            # "tags changed => revision changed": a client holding the pre-strip
            # revision must not treat it as current.
            ...
            stripped.append((slot, key_at_capture or live_key))
        ...
        def _adopt_observed_tags(slot: Any, observed: dict, submitted: dict[str, object]) -> None:
            ...
            # moved by this handler's own publication, so the adopt's meaning is
            # independent of when that publication happens. ``None`` is UNKNOWN and fails
            ...
            if isinstance(observed_tags, list):
                ...
                kept_tags = []
            if kept_tags != list(slot.tags):
                slot.tags = kept_tags
                # "tags changed => revision changed": otherwise a queued PUT holding the
                # pre-adopt revision passes CAS and drops the list adopted here.
                _bump_slot_tags_revision(slot)
        ...
        async def _persist_stripped() -> None:
            for slot, pinned_key in stripped:
                ...
                # Held for the whole write, as on the folder side.
                async with metadata_persist_held(slot) as run_held:
                    await run_held(
                        persist_swept_slot_meta(
                            state,
                            slot,
                            # Captured per slot, so the adoption above can tell whether a
                            # retag landed while we were suspended.
                            {"tags": list(slot.tags)},
        ...
        async def _strip_the_deleted_id_from_folders_and_boards() -> None:
            ...
            for col in state._tag_boards:
                # Same guard the loader uses on this field: a hand-edited non-list is
                # skipped, not coerced, so a truthy scalar cannot reach the comprehension.
