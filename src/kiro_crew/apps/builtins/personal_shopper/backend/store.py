...
class PreferenceStore:
    ...
    def add(self, text: str, *, tags: list[str] | None = None) -> str:
        ...
        with self._lock:
            ...
            if existing is not None:
                self.update(existing.id, text=text, tags=tags)
                return existing.id
    ...
    def add_group(self, name: str, *, icon: str = "") -> str:
        group_id = name.strip().lower().replace(" ", "-")
        with self._lock:
            max_order = self._conn.execute(
                "SELECT COALESCE(MAX(sort_order), 0) FROM groups"
            ).fetchone()[0]
            self._conn.execute(
                "INSERT OR REPLACE INTO groups (id, name, icon, sort_order) VALUES (?, ?, ?, ?)",
                (group_id, name, icon, max_order + 1),
            )
            self._conn.commit()
        return group_id
