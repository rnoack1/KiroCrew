...
async def notify_slot_closed(app: str, slot_key: str) -> None:
    ...
    Never raises and never blocks the close: a dismissal that the app could not
    record still has to remove the tab.
    """
    ...
    try:
        await hook(slot_key)
    except Exception:  # noqa: BLE001 - the close must complete regardless
        logger.warning(
