...
class SessionAllocationService:
    ...
    def supersede_arm_for_new_slot(self, key: str) -> None:
        """Drop *key*'s arm because a NEW slot was just minted on it.

        The teardown verbs cannot make this call. ``destroy`` spends the arm, but the close
        and sweep paths run ``remove``, which preserves it deliberately -- it is still owed
        to a start the cleanup evicts, whose own frame carries only the pre-change directory.
        So the arm legitimately outlives the SESSION while the SLOT is gone, and the next
        occupant of that name is a different slot whose relative writes would silently land
        in the previous one's project.

        Slot creation is the one place that distinguishes them: a freshly minted slot has
        never been armed, so any arm on its key belongs to a previous occupant. Slot REUSE
        returns before this is reached, keeping the retry target ``remove`` preserves.

        The GENERATION is bumped, not dropped: clearing would let a start still in flight
        from the previous occupant compare equal to a fresh key's zero and bind here.
        """
        folded = self._owner._fold_key(key)
        self._key_generation[folded] = self._key_generation.get(folded, 0) + 1
        self._retire_pending_cwd.pop(folded, None)
        self._retire_pending_agent.pop(folded, None)
    ...
    def note_project_change(self, key: str, cwd: str) -> None:
        ...
        self._retire_pending_cwd[folded] = resolved_cwd(cwd, folded)
    ...
    def transfer_retire_arm(self, from_key: str, to_key: str, cwd: str) -> None:
        ...
        arm naming a directory chosen for a binding that no longer exists.
        ...
        carried_agent = self._retire_pending_agent.get(source)
        if source == target and self._retire_pending_cwd.get(target) == resolved_cwd(cwd, target):
        ...
        if source != target:
            self._retire_pending_cwd.pop(source, None)
            self._retire_pending_agent.pop(source, None)
    ...
    async def _reacquire_and_validate(
        ...
        *,
        ...
        pre_armed_alias = self._retire_pending_agent.get(key)
        pre_armed_target = (
        ...
        try:
            async with self._lock:
                ...
                # Non-empty needs no filesystem work, so it stays inline; the CLEARED case
                # uses the value resolved off-thread above, before the lock was taken.
                requested_cwd = (
    ...
    async def get_or_create(
        ...
        **extra_factory_kwargs: Any,
        ...
        cold_armed_alias = self._retire_pending_agent.get(key)
        cold_armed_target = (
        ...
        if stale_generation:
            ...
            return await owner.get_or_create(
                ...
                # The arm holds an ALIAS, so its CURRENT target is the value resolved
                # off-thread above -- a re-point must not resurrect the old agent.
                agent=(
                    (cold_armed_target if cold_armed_alias == retry_agent else retry_agent)
                    or retry_agent
